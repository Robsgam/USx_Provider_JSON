# build_tx_tlets_mc.ps1  -- TX_TLETS v3.0 MC
# Fresh build. 6 cards (Vehicle 1, Person 2, Firearm 1, Article 1, Boat 1).
...
#
# Changes from v2.7:
#   - Vehicle consolidated from 3 cards to 1 (all 10 fields fit)
#   - Person consolidated from 6 cards to 2 (OPTIONS merged into DL; DH separate)
#   - Boat consolidated from 3 cards to 1 (all 6 fields fit)
#   - Added CPL Name combo on DL (name-only search, metadata keyRef CPL)
#   - Removed dead purposeCode from DL card (not in DL metadata)
#   - RSDWW OLN: LIMITATION (identical set[] with DQ OLN, unreachable)
#   - QV VIN on VehReg: LIMITATION (same set[] as RQ VIN, unreachable)
#
# MC Card Layout (6 cards):
#   Vehicle: single card (Plate/VIN/Sticker/FRT/Make/Year/State/RegionId)
#   Person:  DRIVER LICENSE (State/Image/Race/OLN/Name/DOB/Sex/Email/ExpandedDOB/RegionId)
#            DRIVER HISTORY (OLN-DH/Name-DH/DOB-DH/Sex-DH/PurposeCode-DH/Attention-DH/Email-DH)
#   Firearm: single card (Serial/Make/Caliber/NCIC/Image/RelatedHit)
#   Article: single card (Serial/Type/NCIC/Image/RelatedHit)
#   Boat:    single card (RegNum/Hull/NCIC/State/Image/RelatedHit)
...
# =====================================================================
# BUNDLE 1: TX_TLETS PROVIDER (AUTH + QRDM + QMF + 7 QIDMs)
# =====================================================================
...
# --- VehicleStolenQuery (2 combos) ---
# autoSelect=false (officer opt-in), deselects VehReg (one-directional)
...
# --- DriverLicenseQuery (4 combos -- added CPL Name) ---
# Combo order: OLN first (operational priority), then DQ Name (4 set), QW Name (3 set), CPL Name (2 set catchall)
# RSDWW OLN: LIMITATION -- identical set[] with DQ OLN, unreachable
$dlQuery = [PSCustomObject]@{
    ...
    description = 'DriverLicenseQuery -- 4 combos (DQ OLN, DQ Name, QW Name, CPL Name).'; handlerFunction = 'CommsysTransactionRequestHandler'; name = 'TX_TLETS_DriverLicenseQuery'; type = 'QUERYINPUTDATAMAPPING'; autoSelect = $true; provider = 'TX_TLETS'; providerType = 'Commsys'; query = 'DriverLicenseQuery'; queryLabel = 'Driver License'; targetEntity = 'Person'
}
...
$dhQuery = [PSCustomObject]@{
    ...
    description = 'DriverHistoryQuery -- 2 combos. DH-suffix fields. Attention visible.'; handlerFunction = 'CommsysTransactionRequestHandler'; name = 'TX_TLETS_DriverHistoryQuery'; type = 'QUERYINPUTDATAMAPPING'; autoSelect = $true; queriesToDeselect = @('DriverLicenseQuery'); provider = 'TX_TLETS'; providerType = 'Commsys'; query = 'DriverHistoryQuery'; queryLabel = 'Driver History'; targetEntity = 'Person'
}
...
# =====================================================================
# BUNDLE 2: ENTITIES -- MC LAYOUTS (6 cards)
# =====================================================================

# Vehicle -- 1 card: all search fields + options
...
# Person -- 2 cards: DRIVER LICENSE (shared options + DL fields) + DRIVER HISTORY (DH fields)
    ...
    @{
        ...
        rows  = @(
            ...
            @{ id = 'ROW_PER_N2'; cols = @('4','4','4'); fields = @(
                ...
                @{ id = 'emailAddress_Input'; node = Inp 'emailAddress' 'Email Address' '80' 'ROW_PER_N2' }
            )}
            @{ id = 'ROW_PER_N3'; cols = @('4','4'); fields = @(
                @{ id = 'expandedBirthDateSearchCode_Input'; node = Inp 'expandedBirthDateSearchCode' 'Expanded DOB Search' '1' 'ROW_PER_N3' }
                @{ id = 'regionId_Input';                    node = Inp 'regionId' 'Region ID' '4' 'ROW_PER_N3' }
            )}
        )
    }
    @{
        ...
        rows  = @(
            ...
            @{ id = 'ROW_PER_DHN2'; cols = @('4','4','4'); fields = @(
                ...
                @{ id = 'purposeCodeDH_Input'; node = Inp 'purposeCodeDH' 'Purpose Code (DH)' '1' 'ROW_PER_DHN2' }
            )}
            @{ id = 'ROW_PER_DHO1'; cols = @('4','8'); fields = @(
                @{ id = 'attentionDH_Input';    node = Inp 'attentionDH' 'Attention (DH)' '30' 'ROW_PER_DHO1' }
                @{ id = 'emailAddressDH_Input'; node = Inp 'emailAddressDH' 'Email Address (DH)' '80' 'ROW_PER_DHO1' }
            )}
        )
    }
)
$personForm = [PSCustomObject]@{
    description  = 'Person queries -- 2 cards: DRIVER LICENSE (shared opts + DL) + DRIVER HISTORY (DH-suffix).'
    label        = 'Person'
    layout       = $perLayout
    name         = 'ENTITY_Person'
    type         = 'QUERYINPUTFORM'
...
Write-ProviderJson -BundleObject $output -OutPath $OUT -PhasePath $VEROUT `
    -Label "Built TX_TLETS v${Version} MC"

Write-Host ""
Write-Host "Build complete. 6 cards, 24 CommSys combos, 7 QIDMs."
Write-Host "Run build_report.ps1 for full validation."
