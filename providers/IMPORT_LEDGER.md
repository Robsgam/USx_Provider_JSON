...
| IL_LEADS_OFML | usx-il-leads-ofml.mark43.com | DEX-984 | none | v2.1 | **v2.1 is NOT yet imported** — recorded explicitly rather than left silent (`audit_lifecycle` STAGE 6). The tenant EXISTS (URL confirmed by Rob 2026-08-07: `https://usx-il-leads-ofml.mark43.com/rms/#/universal-search`); 0 logs at any version means never installed, NOT that no tenant exists — an earlier revision of this row wrongly said "no USx tenant provisioned", inferred from this ledger's own silence rather than verified. v2.1 is the DEX-1284 convention pass + card collapse (Vehicle/Person/Boat 3 cards → 1, 11 cards → 5; OLN + NCIC Image + Stolen Check canonical labels; ALL-CAPS path-carrying titles; uniform 4/4/4 grid). Layout/label only — zero QIDM/combo/routing change. Capture extension covers this host as of manifest 0.4.0. Awaiting first import |
