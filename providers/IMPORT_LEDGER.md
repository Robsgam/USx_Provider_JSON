...
| HDLE Foundation | HI_HCJDC_OFML | v4.15 | 2026-08-13 | ⚠️ **BEHIND AS OF 2026-08-14 -- the provider tenant is on v4.18 and this one is still on v4.15.** The gap is not cosmetic: v4.16 is what stopped HI discarding the NCIC hit block, so a wanted/missing-person hit returned to HDLE right now still loses its content. **Re-import is Rob's call and is NOT scheduled** -- recorded here so the drift is visible rather than silent. CAD functionality is being tested against this tenant, which is a reason to coordinate rather than swap it underneath them. HI_HCJDC_OFML's first Foundation tenant. **The only row in this table VERIFIED FROM THE TENANT ITSELF rather than from a verbal report** — Rob exported the installed config (`HDLE USx 8.13.26.json`) and it was compared against the repo build with `_json_canonical`: all 5 entity forms identical, all 6 QIDMs identical (44 combination/attribute signature lines, same keyRefs, same ORDER, same set[]/any[]/defaults[]/conditions[]), entire RMS bundle identical, and after normalising explicit nulls EVERY configuration in EVERY bundle hashes identically. Version read from the bundle descriptions: ENTITIES and provider bundle both say `v4.15`. See the verification note below |
| Mariposa Foundation | CA_CLETS | v2.24 | 2026-08-13 | imported by Rob, reported same day. CA_CLETS's first Foundation tenant. **v2.24 CONFIRMED EXPLICITLY 2026-08-14** ("mariposa foundation and live tenants updated to 2.24"), which settles the inference below — the version is no longer assumed. Fully aligned: repo v2.24 · this tenant v2.24 · LIVE v2.24 · DEX-976 release line v2.24 (791400) · catalog v2.24 (2026-08-06). Original note kept for the record: **version was INFERRED** from "the ca clets json", v2.24 being the only CA_CLETS JSON in the repo and the one published on 2026-08-06 |
| Mariposa **LIVE** | CA_CLETS | v2.24 | 2026-08-13 | **v2.24 CONFIRMED EXPLICITLY 2026-08-14** — same statement as the row above; production is on the same version as Foundation, the repo, the release line and the catalog. **⚠ FIRST PRODUCTION INSTALL IN THE PORTFOLIO.** Rob, 2026-08-13, correcting his own earlier note: *"ca clets was importsed to mariposa foundation and live tenants  2 diffrent tenants"* — Foundation and LIVE are two separate tenants, and the row above is only one of them. This is a **THIRD tenant class** that the two-class taxonomy at the top of this file did not contemplate, and it invalidated the standing line "No production environments exist yet". **Exact tenant name/URL NOT reported — this row says "Mariposa LIVE" because that is what was said, not because it is the tenant's name.** Fill it in when known. Version inferred the same way as the row above. **What changes because it is production:** v2.24 is frozen in the strongest sense — real officers are running real CJIS queries against it, so any future CA_CLETS bump is a coordinated live re-import, not a repo action. See the WARNING under section B |
