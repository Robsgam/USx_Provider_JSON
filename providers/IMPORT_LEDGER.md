...
| IL_LEADS_OFML | usx-il-leads-ofml.mark43.com | DEX-984 | **v2.1** (0 logs) | v2.2 | **v2.1 IS INSTALLED on the tenant; repo is now v2.2, so a RE-IMPORT is owed before the sweep.** The v2.1 install is *evidenced, not assumed*: the 2026-08-07 tenant picklist capture came back with rendered labels `NCIC Image`, `Stolen Check`, `Make` and `State (leave blank for IL)` — all v2.1-only strings (v2.0 had `Image (optional)` etc.). Note the "Installed (logged)" column reads **v2.1 (0 logs)**: 0 logs means never tenant-TESTED, which is not the same as never installed — an earlier revision of this row wrongly said "no USx tenant provisioned", inferred from this ledger's own silence rather than verified. v2.2 (2026-08-07) is a cosmetic pass on Rob's rendered-form review: VIN label spelled out + 3 identifier-priority hints removed + Person name order First-then-Last. Label/order ONLY — all three guardrails (`Z2.V` plate>VIN, `Z2.N` OLN>Name, `BQ.R` hull>reg) and the composite Name `sourceField = [NameLast, NameFirst]` LAST-first wire format are untouched. Tenant picklists captured at v2.1 remain valid (no dropdown/codeType change); Vehicle still owes a re-scope. Capture extension covers this host as of manifest 0.4.0 |
