...
| IL_LEADS_OFML | usx-il-leads-ofml.mark43.com | DEX-984 | **v2.2** (0 logs) | v2.2 | **v2.2 IS INSTALLED (Rob, 2026-08-07) — repo and tenant are in sync; sweep can run.** The column reads **v2.2 (0 logs)**: 0 logs means never tenant-TESTED, which is NOT the same as never installed — an earlier revision of this row wrongly said "no USx tenant provisioned", inferred from this ledger's own silence rather than verified. History: v2.1 was installed first (evidenced by a picklist capture returning the v2.1-only labels `NCIC Image` / `Stolen Check` / `Make`), then v2.2 was imported over it. v2.2 is a cosmetic pass on Rob's rendered-form review — VIN spelled out, 3 identifier-priority hints removed, Person name order First-then-Last; label/order ONLY, with all three guardrails (`Z2.V` plate>VIN, `Z2.N` OLN>Name, `BQ.R` hull>reg) and the composite Name `sourceField = [NameLast, NameFirst]` LAST-first wire format untouched. **Tenant picklists were captured WITH v2.2 loaded** (Rob confirmed): all 5 entities, 18/18 dropdowns, 0 capture errors, validation ALL-PASS with 1 inconclusive WARN (`VehicleMakeCode = CNST_FORD` beyond the 300-row DOM cap). Capture extension covers this host as of manifest 0.4.0 |
