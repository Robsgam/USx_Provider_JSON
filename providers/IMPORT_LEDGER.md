...
| HI_HCJDC_OFML | usx-hi-hcjdc-ofml.mark43.com | DEX-1257 | v4.14 (0 current logs) | **v4.15** | **RE-IMPORT OWED, then re-sweep.** v4.15 (2026-08-10, DEX-1283) removes the Attention `'X'` prefill + both KQ/KQN combo defaults — HI was the last provider still carrying it (FL/TX/CA_CLETS/NY all run the same handler with the field in `any[]`, no initialValue, no default, and resolve the officer name on 38/38 DH wires). Wire change, so v4.14's 46 ALL-PASS logs were archived to `_archive_pre_v4.15/` and the package reset to 46 owed. **The one discriminating observation of the re-sweep:** `<Attention>SGAMBELLONE R</Attention>` must still appear on all 9 DH wires — if it comes back ABSENT, the v2.9 gate-feeder theory was right and v4.15 must be reverted |
