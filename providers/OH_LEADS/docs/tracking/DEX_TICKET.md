# OH_LEADS â€” DEX Ticket

**Active ticket:** [DEX-990 â€” \[OH - LEADS\] USx Provider Build](https://mark43.atlassian.net/browse/DEX-990)
...
automation-authored (ðŸ¤–) comments may ever be edited: never Rob's own manual notes, never a third
party's.

> **THIS FILE IS NOT THE TICKET â€” read the ticket.** Verify with `getJiraIssue DEX-990`
> (cloudId `5ba7ec1f-1b3f-4b21-a2f2-5d04d124de2c`). `audit_lifecycle` (enforce 2r) READS THIS FILE,
> NOT THE TICKET â€” it can PASS on a lie and FAIL on the truth. HI's equivalent file did exactly that.
>
...
was no repo source to copy â€” and inventing a plausible number is a mistake already made once on IL
...
**Posted so far:** NOTHING. `comments: []`, total 0 â€” this ticket has never been commented on, by
...
POSTED: v2.11 comment 803603 2026-08-21
...
**Current: v2.11 -- TENANT-VERIFIED ALL-PASS 5/5 (65 logs, 65 PASS / 0 FAIL) 2026-08-21.** WIRE change:
the 8 name components joined the any[] of every name combo (DQ.N, QWA, DN, KQ.N, RN) -- previously
composed into Name but in NO set[]/any[], so an officer's middle name went nowhere and the v2.10 wire
read "DOE, JOHN" on all 56 logs. Now PROVEN on all THREE name pools (Person DL, Person DH, vehicle
owner): "DOE, JOHN A JR". The pre-existing wires are proven UNCHANGED, not assumed -- 53 of 56
archived v2.10 logs byte-identical, and the 3 that differ (RN_any, DQ.N_any, KQ.N_any) differ in the
<Name> element ALONE, which is exactly what pooling the components should do to a fill-every-optional
test. Supersedes the v2.7 deferral, which asked for a live middle-name query to settle it -- that was
unreachable by construction, since an unpooled field generates no test. One redundant capture retired:
ATDP's guardrail submitted form state byte-identical to its own combo test (LicensePlateYear=2026 is
the form prefill), so it staged no contest; fixed at the generator, log archived, inflation now 0/0/0/0.
Release line **comment 803603**; 802507 retained as v2.10 history, 801113 is the history anchor.
...
- **v2.4** â€” **card collapse 14 â†’ 6** (Vehicle 6â†’1, Person 5â†’2, Boat 3â†’1) with every title now
  carrying its query paths; OH had been the only provider of nine with 0 path-carrying titles. Zero
  routing change. Helper labels fell 18 â†’ 2 as a *consequence* (the survivors were saying which combo
  needs a field â€” a card title's job). Plus **ImageQuery BUILT**, the last unbuilt devdoc-Basic query:
  ...
  **Driver Photo** registered in `verify_build` and the CLAUDE.md table. Extract promoted
  PROVISIONAL â†’ **CONFIRMED** (gating): 23 PASS / 0 FAIL / 0 WARN, 7 of 7 Basic queries built.
- **v2.3** â€” DEX-1283 Attention `'X'` removed (feeder + both KQ.N/KQ.O combo `defaults[]`) and a
  DEX-1284 label pass: `License Number` â†’ **OLN** (OH was the only provider of nine not using the
  canonical label), three different image labels â†’ **NCIC Image**, three verbose stolen-check labels
  â†’ **Stolen Check**, `(optional)` suffixes stripped, and four `# LABEL-OVERRIDE:` tags added so the
  lean labels register as INFO rather than CHECK 15 WARNs. Cleared
  `[FLAG:validate-imgind-20b-l30]` â€” validator recorded 78 PASS where STATUS.txt was stuck at 77P.

**Owed to this ticket:** the full changelog dump (v1.0 â†’ v2.4), then a release line once the sweep
passes. **Jira is DRAFT-AND-WAIT â€” draft and wait for Rob's explicit approval before posting.**

**BOTH OPEN ITEMS ARE CLOSED at v2.4 (Rob's call 2026-08-10: "fix 1 and 2 â€” that was the point").**
1. ~~`ImageQuery` devdoc-Basic and NOT BUILT~~ â†’ **BUILT.** `Set[OperatorLicenseNumber]` exactly per
...
2. ~~14 cards, 0 path-carrying titles~~ â†’ **6 cards, 6 path-carrying titles**, matching the portfolio.
   Person deliberately stays at TWO cards (DL + DH) because the DH-suffix fieldIds are a separate
   field pool â€” that is the isolation mechanism, and HI/NY/TX/AZ keep the same split.

**One advisory remains, pre-existing and NOT introduced by v2.4:** `audit_requirement_fidelity`
reports 1 UNDER-REQUIRED (`DQ.O` missing `SocialSecurityNumber`). It is the shared-keyRef artifact â€”
