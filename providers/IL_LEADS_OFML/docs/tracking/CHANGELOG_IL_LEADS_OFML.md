...
Current: **v2.8** | Generated: 2026-09-14
...
WHY: Rob 2026-08-13 -- "on personn  shortne theoln field  the label is squsiehed  oln has a much  
  smaller max size thent he field appears".  
CHECKED FIRST -- THIS IS NOT A CAPACITY DEFECT. IL metadata declares  
  <Field name="OperatorLicenseNumber" ... maxLength="20"> (7 occurrences in the XML, every one  
  20) and the built control declares maxLength 20. They agree exactly, so there is no  
  QIF-exceeds-XML problem and audit_metadata is correct to stay silent. Worth stating because  
  the CA_SAN_LUIS_OBISPO case (QIF 20 > XML 17, "server may reject") looks identical from the  
  form and IS a real defect -- the metadata is what separates them.  
WHAT WAS ACTUALLY WRONG: width. usx-cosmetic's rule is that width tracks EXPECTED INPUT, not  
  maxLength; an IL OLN runs ~12 characters, so 6 of 12 columns oversold it. The knock-on is what  
  Rob actually saw: with OLN at 6, the three context selects had only 2 columns each and  
  "State (leave blank for IL)" cannot render in 2 columns. Equal quarters give those selects 50%  
  more width. Still sums to 12 (L6).  
COST: the 14 Vehicle logs captured at v2.6 are archived. Vehicle's entity fingerprint is  
  UNCHANGED by this edit (only Person moves), but block-by-version is deliberate policy -- every  
  bump retests all five entities -- so they re-run. 41 automated tests.  
...
WHY: Rob 2026-08-13, verbatim -- "cosmetic  stolen check on per needs to go on top line".  
**CHANGED** (layout only -- no QIDM, no combination, no default value, no label touched):
    ROW_PER_1  cols 6/3/3 -> 6/2/2/2, gains relatedHitSearchIndicator  
    ROW_PER_4  retired -- it held only Stolen Check, alone on a 4-column row  
WIDTHS, and why these: the OLN identifier keeps 6 and the three context selects (State,  
  NCIC Image, Stolen Check) share the remaining 6 EQUALLY so they read as one group. That  
  preserves the shape of NJ_NJCJIS ROW_PER_1 -- 6/3/3 OLN + State + NCIC Image -- which is the  
  only 0-finding provider in audit_layout_flow and therefore the layout standard. Checked  
  OH_LEADS too; neither clean reference has a 4-control top row, so this EXTENDS their pattern  
  (identifier wide, context narrow, row sums to 12) rather than inventing a new one.  
TIMING, deliberate: taken NOW, mid-sweep, rather than after. A cosmetic bump archives the whole  
  test package either way, so doing it after the 41-test sweep would have cost all 41 logs  
  instead of the 14 Vehicle logs captured at v2.4. Re-test cost is never grounds to DEFER a fix  
  (Rob 2026-08-11), but when a bump is happening regardless the cheaper moment is still the  
  right one.  
COST: the 14 v2.4 Vehicle logs (ALL-PASS, ingested 2026-08-13) are archived. Sweep restarts at  
  T1 for all 41 tests on v2.5.  
NOT FIXED, reported instead -- audit_layout_flow [L9] on ROW_PER_3: 'raceCode' feeds the RMS  
  query ONLY (it is in NO CommSys combination) yet shares a row with the mandatory identifier  
  BirthDate, so it reads as though it queries the state. Pre-existing, unrelated to this change,  
  and outside what Rob asked for. Needs his call, not a silent fix.  
