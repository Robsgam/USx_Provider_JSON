# validate_az_azdps.ps1
# Structural validator for AZ_AZDPS.json
#
# Checks performed:
...
#
# Update $expectedQifCount once entity form count is known from XML.
# Update $knownUnmapped list after confirming intentionally unmapped fields.
#
# Usage:
#   powershell.exe -ExecutionPolicy Bypass -File scripts\validate_az_azdps.ps1
#   powershell.exe -ExecutionPolicy Bypass -File scripts\validate_az_azdps.ps1 -JsonFile "path\to\other.json"

param(
    [string]$JsonFile = "$PSScriptRoot\..\AZ_AZDPS.json"
)
...
Write-Host " AZ_AZDPS JSON VALIDATOR" -ForegroundColor White
...
# AZ_AZDPS provider QIDMs only (excludes RMS QIDMs)
$qidms = @($allConfigs | Where-Object { $_.type -eq "QUERYINPUTDATAMAPPING" -and $_.provider -eq "AZ_AZDPS" })

# All QUERYINPUTFORM configs (ENTITIES bundle)
$qifs  = @($allConfigs | Where-Object { $_.type -eq "QUERYINPUTFORM" })

Write-Host " QIDMs (AZ_AZDPS): $($qidms.Count)"
...
# ---------------------------------------------------------------------------
# QIF count check -- UPDATE $expectedQifCount once entity forms are designed
# ---------------------------------------------------------------------------
Write-Header "CHECK 0 -- ENTITIES Bundle QIF Count"

$expectedQifCount = 5   # Phase 1: Vehicle, Person, Firearm, Article, Boat (single card each)
if ($expectedQifCount -eq 0) {
    Write-Warn "QIF count check skipped -- update `$expectedQifCount in validator once forms are designed"
    Write-Info "Present QIFs ($($qifs.Count)): $(($qifs | ForEach-Object { $_.label }) -join ', ')"
} elseif ($qifs.Count -eq $expectedQifCount) {
...
foreach ($qidm in $qidms) {
    ...
    # Build sourceField lookup: sourceFieldValue -> attributeName
    $sfLookup = @{}
    ...
    if (-not $qidm.combinations -or $qidm.combinations.Count -eq 0) {
        ...
        if ($dupes) {
            foreach ($d in $dupes) { Write-Fail "Duplicate keyReference: '$($d.Name)' -- platform rejects import with 'Duplicate key references found in query input data mapping'" }
        } else {
...
# Known-intentional unmapped fields -- update once build is underway
$knownUnmapped = @(
    "CAD_UNIT_SELECT_VALUE",   # CAD integration field -- never in QIDM by design
    "CAD_EVENT_SELECT_VALUE",  # CAD integration field -- never in QIDM by design
    "RegistrationState"        # Hidden SelH for RMS only (attributeTypeId=STATE); not in AZ QIDM
)
