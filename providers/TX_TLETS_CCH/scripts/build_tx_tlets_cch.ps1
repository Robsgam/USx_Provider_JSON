    ...
    @{
        ...
        rows  = @(
            ...
            @{ id = 'ROW_VEH_1'; cols = @('3','3','3','3'); fields = @(
                ...
                # v1.19 -- BASE-SYNC with TX_TLETS v4.23. NO 'TX' PREFILL, matching the base.
                # A TX default was tried on Vehicle and REVERTED the same hour: enforce found that
                # with State prefilled, a devdoc #1 fill (Plate + PlateYear) is swallowed by
                # QV{Plate,State}, which carries no LicensePlateYear anywhere, so the officer's year
                # is silently discarded. Full reasoning in the BASE script at ROW_VEH_1.
