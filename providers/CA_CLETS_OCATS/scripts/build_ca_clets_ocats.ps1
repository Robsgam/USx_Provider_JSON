...
# v2.10 WITHDRAWS v2.9 AND RESTORES THE v2.8 CONFIGURATION EXACTLY. Rob 2026-09-01: "revert to v2.8
# shape." v2.9 hid and handler-automated the query-level `userId`, merged devdoc Veh #1 into #2 and
# re-ordered the in/out pairs. It was withdrawn because the AUTHORITY does not support the mechanism:
#   The devdoc's Routing and Configuration section (L30-31) says "The CLETS assigned User ID should be
#   placed in the <Authentication>/<UserName> field in the ConnectCIC header" -- and Build-Auth ALREADY
#   does exactly that, with CommsysGetDexStateUserIdRuleHandler on the AUTH attribute. So the CLETS
#   User ID is already transmitted, in the right place, by the documented mechanism.
#   The QUERY-level `UserId` is therefore a DIFFERENT field: maxLength 2 in ALL SEVENTEEN transactions
#   that define it (checked per-transaction, so this is not the wrong-transaction read that got
#   Authorization wrong at v2.1). Putting the AUTH handler on it would emit the 6-character DEX
#   username into a 2-character field. Rob spotted it: "user id at 2 characters is strang ... we may be
#   chasing this in the wrong way."
#   NO VALID VALUE FOR THE QUERY-LEVEL FIELD IS KNOWN TO THIS REPO, so devdoc Veh #2 and DL #1 cannot
#   be made to succeed by any change here -- and combos that cannot succeed must not be the ONLY path
#   for a search. Retiring '4' had made AWVEHQ the sole in-state plate combo, so withdrawing is the
#   safe direction: v2.8 keeps '4' and leaves userId a VISIBLE control the officer can fill.
#   TO REVISIT, ONE FACT IS NEEDED: does the agency have a 2-character OCATS User ID, and is it
