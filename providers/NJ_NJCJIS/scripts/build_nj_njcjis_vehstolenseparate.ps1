...
$PHASEDIR = "$DIR\phases"
$OUT      = "$DIR\NJ_NJCJIS_VehStolenSeparate.json"
...
# =====================================================================
# 1d. VehicleRegistrationQuery -- UNCHANGED from mainline
...
$vehicleForm = [PSCustomObject]@{
    description  = 'Vehicle queries -- VehStolenSeparate branch: OPTIONS + PLATE + VIN + "Vehicle Stolen" cards. Stolen triggers suffix-isolated on the Vehicle Stolen card.'
    label        = 'Vehicle'
    layout       = $vehLayout
    name         = 'ENTITY_Vehicle'
    type         = 'QUERYINPUTFORM'
...
Write-ProviderJson -BundleObject $output -OutPath $OUT -PhasePath $VEROUT `
    -Label "Built NJ_NJCJIS v${Version} (VehStolenSeparate experimental branch)"
