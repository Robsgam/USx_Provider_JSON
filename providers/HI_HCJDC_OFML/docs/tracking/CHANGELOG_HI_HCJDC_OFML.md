...
Current: **v4.20** | Generated: 2026-08-17

---

## v4.20 -- 2026-08-17 -- FIX v4.19's OVER-PERMIT: name parts removed from the OLN DH combo (KQN)

**CHANGED:** Split the shared DH any[] list in two. `$dhAnyCommon` (RegistrationStateDH, purposeCodeDH,
  Attention) goes to BOTH DH combos; `$dhAnyName` adds nameMiddleDH + nameSuffixDH and goes to the  
  NAME combo (KQ) ONLY. KQN's any[] no longer carries the name parts.  
**REASON:** *** v4.19, SHIPPED EARLIER THE SAME DAY, PUT THE NAME PARTS IN BOTH DH COMBOS AND THAT
  REACHED THE WIRE AS A MALFORMED NAME. *** DriverHistoryQuery has two combinations and only one is  
  a name search:  
      KQ  set=[SexCodeDH,BirthDateDH,NameLastDH,NameFirstDH] -> metadata KQ{Name},  
          Set=[SexCode,BirthDate,Name]  
  ...
  The OLN variant defines NO Name field AT ALL -- not in Set, not even in Any. v4.19 added the two  
  components to the SHARED $dhAny, so KQN inherited them, and an OLN-only DH query with a middle  
  initial typed in composed a name out of whatever was present. The v4.19 tenant sweep captured it  
  on three real wires:  
      KQN  <Name>A JR</Name>      <Name>JR</Name>      <Name>A</Name>  
  A driver-history query carrying a "name" with no surname and no first name.  
*** THE GATE CAUGHT THIS AND I DID NOT RUN IT. *** audit_requirement_fidelity reported it exactly:  
  "[WARN] DriverHistoryQuery / KQ -> built 'KQN' OVER-PERMITTED: Name; Name", 2 OVER-PERMITTED on  
  14 branches compared -- right combo, right field, right direction. After building v4.19 I ran the  
  validator, audit_name_components and the package reset, but NOT fidelity. This is the THIRD time  
  in one day a gate fired and went unread (firearmMake's trust warning on IL; TN_TIES's 22 trust  
  issues including three mandatory set[] fields with no test value; now this). The gates are not the  
  weak link -- reading them is. After a build, run the fidelity gate before the sweep, not after.  
VERIFIED: audit_requirement_fidelity 2 OVER -> 0 OVER with the denominator UNCHANGED at 14 branches  
  compared (a suppression would have dropped the count instead). audit_name_components still 8  
  components / 0 findings, so the v4.19 capability fix is intact. validator 65P/0F/0W.  
WHAT v4.19 GOT RIGHT AND THIS KEEPS: the KQ name search and the DL DQ search now genuinely emit all  
  four name components -- the v4.19 sweep captured <Name>DOE, JOHN A JR</Name>, DOE, JOHN JR and  
  DOE, JOHN A on both DQ and KQ, which no HI log had ever carried before.  
*** C3 IS STILL NOT SETTLED, AND I WILL NOT CLAIM IT IS. *** The v4.19 sweep was supposed to decide  
  whether any[] membership is REQUIRED for a composite sourceField to reach the wire (HI was the  
  only provider isolating the variable: controls present, composed, zero any[]). It cannot: v4.18  
  never had a test that FILLED middle or suffix -- the resolver had them frozen to null until  
  2026-08-17 -- so there is no v4.18 wire to compare against. What is proven is only that the  
  current shape (control + sourceField + any[]) works. audit_wiring_closure's class A may still be  
  too narrow; deciding that needs a deliberate probe on a provider still in the composed-but-not-in-  
  any[] state, and after this fix HI is no longer one.  
COST: 16 v4.19 Vehicle logs + 20 v4.19 Person logs archived; all 5 entities restart from T1. The 3  
  malformed-name Person logs are archived with them -- they are the EVIDENCE for this entry, not a  
  regression to carry forward. HDLE LIVE unaffected (deliberately held at v4.15).  

## v4.19 -- 2026-08-17 -- middle name + suffix reach the wire (any[] wiring) + MI label capacity fix
...
**REASON:** Found by tools\audit_name_components.ps1 (new 2026-08-17) -- the authority->built gate at
  ...
  is absent -- no double space, no stray comma.  HI WAS A DIFFERENT AND MORE MISLEADING FAILURE THAN THE OTHERS -- the only provider in this state.  
  The controls EXISTED and were visible, the composite Name attribute ALREADY listed all four  
  components, and the separators were ALREADY the correct triple. The two components simply sat in  
  NO combination's any[], so an officer could type them on both the DL and DH cards and neither  
  value could reach the wire. Fix: nameMiddle/nameSuffix -> DQN's any[]; nameMiddleDH/nameSuffixDH  
  -> $dhAny, BOTH arms (the Attention-diagnostic build must not differ by a second variable).  
THIS ALSO SETTLES AN OPEN QUESTION THAT WAS NOT ANSWERABLE FROM THE REPO. audit_wiring_closure  
  reported HI CLOSED, because its class A requires "no attribute sources it AND no combo references  
  it" -- attribute-sourcing alone counted as reaching the wire. I claimed the opposite. Neither  
  position had a committed log, so audit_name_components records it as C3 NOT-IN-POOL, a [NOTE] and  
  not a FAIL, under hypothesis quarantine. HI was the ONLY provider isolating the variable. The  
  upcoming sweep decides it: if <Name> now carries middle+suffix where v4.18's 46 logs never could,  
  any[] membership was load-bearing and C3 should become BLOCKING. Read the result before promoting  
  it -- this fix landing does not by itself prove the diagnosis.  
COSMETIC, AND IT REVERSES AN EARLIER DELIBERATE DECISION -- stated rather than quietly overwritten:  
  an earlier label pass changed nameMiddle/nameMiddleDH from 'Middle Name' to 'M.I.'/'MI'. Both  
  fields are maxLength=30, i.e. a full middle name, so 'MI' claims a single initial and  
  misrepresents capacity (rule L7). AZ_AZDPS made exactly this correction at v3.10. Reverted to  
  'Middle Name' with a LABEL-OVERRIDE tag on each. The operator has final say -- if 'M.I.' is  
  wanted back, cap the fields at 1 or record the override.  
COST: all 5 entities re-open; 46 logs archived. HDLE LIVE is UNAFFECTED -- it is deliberately held  
  at v4.15 and this does not change that hold.  
...
WHY NOW, and this is a correction of my own call: I built v4.16 without taking the flag, on the  
  grounds that Rob had approved "only adding response handling". He pushed back -- "why not  
  address this now?" -- and he was right. The bump had ALREADY happened: v4.16 archived 46 logs,  
  a re-sweep and an HDLE re-import were already owed. Taking the flag here costs NOTHING extra,  
  while deferring buys a SECOND bump, archive, sweep and re-import for a five-site change. That  
  is the same reasoning I had applied to IL_LEADS_OFML v2.5 hours earlier and failed to apply  
  here. The error was not holding scope -- it was not OFFERING the option.  
MEASURED BEFORE APPLYING, as the flag demands:  
  - ImageIndicator is any[]-ONLY on all four VehReg combos: 0 set[], 0 conditions. Routing risk  
    ZERO, so no prefill can hide a combination (BUILD_RULES 24).  
  - Only ONE control exists on this provider -- ENTITY_Vehicle. Person, Firearm, Article and  
    Boat have NO ImageIndicator control and no combo carrying it, so "everywhere" means VEHICLE  
    here. Adding controls to the other four would OVER-PERMIT, the same adjudication that made  
    the flag a no-op on CA_CLETS and excluded Article on IL.  
  - TX_TLETS T6 gate CLEAR: HI's devdoc contains no "must be filled if X = Y" wording at all.  
**CHANGED**, form + CAD twin together (a form initialValue alone is HALF a default -- CAD ignores
  form initialValue): Vehicle form 'N'->'Y', and defaults[] 'N'->'Y' on RQ, RQV, M55L, M55S.  
  Five sites. Verified with a DIFFERENT pattern than the edit used: zero ImageIndicator sites  
  remain on 'N'.  
SCOPE: this is a REQUEST-side change (form default + combo defaults), unlike v4.16 which was  
  response-only. Combination set[]/any[]/conditions are still untouched; only default VALUES move.  

## v4.16 -- 2026-08-13 -- NCIC hit-block response handling -- +24 QRDM attributes. ZERO request change.

WHY: Rob 2026-08-13 -- "we cannot be discarding infomation like hit warnings regadles of  
  ccntent". A LIVE capture from HDLE Foundation (tx 01KZY7M48VXBWV1CBRJ6PDVEA7) returned an NCIC  
  WantedPerson record on a DriverLicenseQuery, and 23 of its 35 elements mapped NOWHERE --  
  MessageKey "WANTED PERSON - CAUTION", CautionMedicalCondition "00 - ARMED AND DANGEROUS",  
  ExtraditionLimitation, NCICNumber, FBINumber, WarrantDate, 30 AKAs. The officer saw a hit with  
  every reason it was dangerous stripped out.  
FOUND WHILE SCOPING, WORSE THAN THE ORIGINAL REPORT: the generated QRDM maps Hit as a SCALAR  
  (Hit -> hit), but the live response makes it a CONTAINER --  
  <Hit><Banner>NCIC WANTED PERSON</Banner><Detected>1</Detected></Hit>. So the banner itself had  
  nothing pointing at it. simulate_response did not catch it because its synthetic corpus also  
  models <Hit>Y</Hit> as a scalar -- the tool agreed with the bug.  
**CHANGED:** 24 attributes appended to HI_HCJDC_OFML_Results in three tiers -- safety
  (Hit.Banner, Hit.Detected, MessageKey, CautionMedicalCondition, ExtraditionLimitation,  
  MessageData, Class), identity (NCIC/FBI/SID numbers, WarrantDate, OffenseCode, ORI + case  
  number, AlsoKnownAs x2), descriptive (skin/SMT/misc/POB/DNA/validation/FreeText/ResponseType).  
PASS-THROUGH ONLY, NO CODE-MAPPING HANDLERS -- deliberate. The live record carries  
  EyeColorCode 'BR0' with a DIGIT ZERO (the Illinois ORI writes 0 for O throughout: J0HN,  
  JER0ME, R0BB), so an attribute-mapping lookup would miss and BLANK the field. Raw beats blank  
  when the content is a caution flag.  
MessageData IS THE SAFETY NET: the structured elements expose only AlsoKnownAs/AlsoKnownAs2 while  
  the real record held 30 AKAs and 8 alias DOBs in text alone. Mapping it shows what we have not  
  modelled -- which is what "regardless of content" requires.  
ResponseType IS THE SUPPRESSION DISCRIMINATOR: one DL query returns FOUR blocks (2x SWITCH_ACK  
  transport acks, NCIC, ADLA); without it one query renders as four results.  
HI-ONLY ON PURPOSE. Build-CommsysQrdm is shared by all 20 providers and every one carries the  
  identical 67-attribute DL-shaped map -- so does engineering's hand-built Lafayette LA_LEMS JSON  
  (0 of 21 mapped). That symmetry says product-level gap, not HI omission. A shared-module fix  
  would move 20 providers at once INCLUDING a live production tenant (CA_CLETS at Mariposa).  
  Promote to Build-CommsysQrdm only after the rendering is confirmed on HI's own test tenant.  
ADDITIVE ONLY -- PROVEN ON A REPLICA BEFORE BUILDING, not asserted: all 5 QIFs, all 6 QIDMs,  
  AUTH, QMF and the entire RMS bundle canonically IDENTICAL; all 67 original QRDM attributes  
  survive byte-identical; 12 combinations before and after with 0 differences. The wire XML is  
  unchanged -- only what we keep from the reply moves.  
COST: archives 46 ALL-PASS logs, full re-sweep, HDLE Foundation re-import. The sweep emits  
  byte-identical wire XML to v4.15; it re-runs for ATTRIBUTION, not because the request moved.  
CANNOT BE VERIFIED FROM THE REPO: QRDM output resolves platform-side into the RMS UI and never  
  appears in captured wire, and simulate_response is synthetic-only with no wanted-person block.  
  Confirm by eye on HI's test tenant with a query that returns a real hit, and confirm the dotted  
  Hit.Banner syntax there -- dotted paths are proven on the RMS side (AZ uses  
  eyeColorAttrDetail.displayValue) but not yet on a CommSys XML response.  
