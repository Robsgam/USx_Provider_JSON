# validate_ny_nyspin_ejustice.ps1
# Structural validator for NY_NYSPIN_EJUSTICE.json
#
# Checks performed:
...
#
# Usage:
#   powershell.exe -ExecutionPolicy Bypass -File scripts\validate_ny_nyspin_ejustice.ps1
#   powershell.exe -ExecutionPolicy Bypass -File scripts\validate_ny_nyspin_ejustice.ps1 -JsonFile "path\to\other.json"

param(
    [string]$JsonFile = "$PSScriptRoot\..\NY_NYSPIN_EJUSTICE.json"
)
...
Write-Host " NY_NYSPIN_EJUSTICE JSON VALIDATOR" -ForegroundColor White
...
# NY_NYSPIN_EJUSTICE provider QIDMs only (excludes RMS QIDMs)
$qidms = @($allConfigs | Where-Object { $_.type -eq "QUERYINPUTDATAMAPPING" -and $_.provider -eq "NY_NYSPIN_EJUSTICE" })

# All QUERYINPUTFORM configs (ENTITIES bundle)
$qifs  = @($allConfigs | Where-Object { $_.type -eq "QUERYINPUTFORM" })

Write-Host " QIDMs (NY_NYSPIN_EJUSTICE): $($qidms.Count)"
...
# Known-intentional unmapped fields
$knownUnmapped = @(
    ...
    "RegistrationState"        # Hidden SelH for RMS only (attributeTypeId=STATE); mapped via RMS QIDM, not NY QIDM
)
