...
$vehRegQuery = [PSCustomObject]@{
    ...
    combinations = @(
        [PSCustomObject]@{
            requirements          = [PSCustomObject]@{ set = @('LicensePlateNumberIn','LicensePlateTypeCode','LicensePlateYear','RegistrationState'); any = @('ImageIndicator') }
        ...
        [PSCustomObject]@{
            requirements          = [PSCustomObject]@{ set = @('LicensePlateNumberIn'); any = @('LicensePlateYear','ImageIndicator') }
            primaryFieldReference = 'LicensePlateNumberIn'
...
$vehStolenQuery = [PSCustomObject]@{
    ...
    combinations = @(
        [PSCustomObject]@{
            requirements          = [PSCustomObject]@{ set = @('LicensePlateNumberIn'); any = @('ImageIndicator','RegistrationState','VehicleMakeCode') }
            primaryFieldReference = 'LicensePlateNumberIn'
    ...
    @{
        ...
        rows  = @(
            @{ id = 'ROW_OPT'; cols = @('6','6'); fields = @(
                @{ id = 'RegistrationState_Input'; node = Sel 'RegistrationState' 'State (OOS DL / all DH)' @{ attributeTypeId = 'STATE' } 'ROW_OPT' }
                @{ id = 'ImageIndicator_Input';    node = Sel 'ImageIndicator' 'Image' @{ codeTypeCategory = 'YES_NO_UNKNOWN'; codeTypeSource = 'NIBRS'; initialValue = 'Y' } 'ROW_OPT' }
            )}
        )
    }
    @{
    ...
    @{
        ...
        rows  = @(
            @{ id = 'ROW_DH1'; cols = @('6','6'); fields = @(
                ...
                @{ id = 'PurposeCode_Input';              node = Inp 'PurposeCode' 'Purpose Code' '1' 'ROW_DH1' @{ initialValue = 'C' } }
            )}
