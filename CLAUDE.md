...
## Provider Status (updated 2026-06-12)
...
| FL_FCIC | providers/FL_FCIC/ | v4.9 | 90P/0F/0W/0LIM -- 31/31 combos, 6 QIDMs -- v4.9 NOT yet imported; test package reset to T1 (v4.8 matrix: 6 PASS then DH State=FL FAIL -> inert-guard fix) | 2-card: Vehicle(Options+Search), Person(DL+DH OOS-only), Boat(Options+Search). Devdoc combo order + routing conditions (State NOT_EXISTS / OLN NOT_EXISTS / StolenSearch EQUALS-NOT_EQUALS Y) for first-match + pool isolation, DH KQ out-of-state only (FCIC: State in set[], NOT_EQUALS FL, no default), DH+Boat destination state = blank 2-char FormInput no codeTypeProvider (INERT-CONDITION fix v4.9 -- value guards need literal values), BQ x3 restored, QV pending data-mined confirmation, one-directional queriesToDeselect, Attention visible, RMS Vehicle stripped to 3 attrs, RMS union-pool over-send fix designed but GATED (G1 impact / G2 handler verification) |
