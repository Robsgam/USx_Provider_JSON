...
## Provider Status (updated 2026-07-15)

| Provider | Path | Version | Status | Notable patterns |
|---|---|---|---|---|
| NJ_NJCJIS | providers/NJ_NJCJIS/ | v4.8 | 61P/0F/0W/0LIM -- v4.8 (2026-07-01) metadata-driven keyRef rename (DQ/DQN->FULL/FULLN, RQ/RQN->RANDFULL/RANDFULLN per devdoc). LIVE-TESTED: all 5 entities block_entity-locked at v4.8 (.test_state.json = authority). Full history: `providers/NJ_NJCJIS/docs/tracking/CHANGELOG_NJ_NJCJIS.md`. | VehicleStolenQuery NOT built (USER-APPROVED skip; state auto-runs QV, response data-mined via QRDM); VehReg 2 combos (RANDFULL/RANDFULLN), poisoned-array RandomRequest=Y conditions removed (RandomRequest user-controlled in any[]); DriverLicense 2 combos (FULL/FULLN); PascalCase USx fieldIds (CAD/OnScene), Mark43/RMS keys stay camelCase; CAD combo defaults; NCIC state; shared RMS module, RMS Vehicle stripped to 3 attrs |
| HI_HCJDC_OFML | providers/HI_HCJDC_OFML/ | v4.7 | 66P/0F/0W/0LIM -- v4.7 (2026-07-02) NJ/CA-parity modernization (retired phases/, migrated docs/ to 4-category structure, tests/->logs/<Entity>/) + Vehicle OOS-gate symmetry fix (LicensePlateTypeCode EXISTS on RQ, RegistrationState EXISTS on RQV, mirrors M55L/M55S NOT_EXISTS gates). LIVE-TESTED: all 5 entities block_entity-locked at v4.7 (.test_state.json = authority). Full history: `providers/HI_HCJDC_OFML/docs/tracking/CHANGELOG_HI_HCJDC_OFML.md`. | 6 basic queries (Article/Boat/DH/DL/Gun/VehReg), 17 combos all reachable; single JSON; Person 3 cards (Search Options[State] + Driver License + Driver History, DH-suffix, one-directional deselect); Vehicle/Boat/Firearm/Article 1 card; Type Code dropdown (VEHICLE_TYPE/HI_NIBRS); ImageIndicator=N combo defaults on all VehReg combos; State in DL/DH any[] + VehReg any[] for OOS; identifier-priority guardrails complete (Plate>VIN, OLN>Name DL+DH, Hull>Reg); Name Last-first (v4.0) |
| NY_NYSPIN_EJUSTICE | providers/NY_NYSPIN_EJUSTICE/ | v4.7 | 81P/0F/0W/0LIM -- v4.7 (2026-07-13) ImageIndicator default 'Y'->'N' for Vehicle/Article/Boat entities (form field initialValue + all their combo defaults); Person (DL/DH) ImageIndicator stays 'Y'. Rebuild reset the USx Tenant Testing package -- Vehicle/Article/Boat back to PENDING; Firearm+Person preserved (unchanged fingerprints). v4.6 (2026-07-07) collapsed Vehicle + Boat each to a SINGLE card (SEARCH OPTIONS folded into the identifier row); DL/DH labels path-tagged for clarity ("(Name search)" on name-path fields, "(optional)"/"(opt)" on fully-optional, License Number notes the Name alternative). Layout/label only. Tenant picklists captured this session (docs/reference/TENANT_PICKLISTS.json, all 5 entities -- one-time provider scope). v4.5 Person tighten (dumped OPTIONS, DH self-contained w/ own RegistrationStateDH+ImageIndicatorDH); v4.3 restored MessageContinueKeyCode (metadata is field-authority); DGRP own card (last), full 10-field set, autoSelect=TRUE. v4.1 removed Gun ImageIndicator + Boat RelatedHitSearchIndicator (NOT in metadata). LIVE-TESTED: full v4.6 pass re-run via extension Run Plan (72/72 PASS, 0 FAIL), all 5 entities block_entity-locked at v4.6 (.test_state.json = authority). Full history: `providers/NY_NYSPIN_EJUSTICE/docs/tracking/CHANGELOG_NY_NYSPIN_EJUSTICE.md`. | PascalCase, 6 cards (Veh 1, Per 3 [DL+DH+DL-Name-Search], Gun 1, Art 1, Boat 1), 17 combos, 7 QIDMs, DGRP own card (last)+full 10-field metadata set (autoSelect), DH self-contained w/ own StateDH/ImageDH + OOS combos (DALHOUT/DALLOUT) RegistrationStateDH EXISTS/NOT_EXISTS routing, Vehicle plate OOS combo (RVEHOUT), NyNyspinTransactionName visible on DH (default DALL), PurposeCode default C on DH OOS combos, Choice-set OOS pattern (LIMIT #36), DH-suffix+one-directional queriesToDeselect, CAD defaults, State no-default (LIMIT #30), identifier-priority guardrails (Plate>VIN, OLN>Name DL+DH, Hull>Reg) |
...
| FL_FCIC | providers/FL_FCIC/ | v7.1 | 92P/0F/0W/0LIM -- 31/31 combos, 6 QIDMs. v7.1 (2026-06-30) Boat Hull>Reg guardrail completed for the FBQ in-state family (last of the Hull/Reg union over-send fixes; QB/BQ families fixed v6.9). Zero-error live pass certified (DEX-971). Full history: `providers/FL_FCIC/docs/tracking/CHANGELOG_FL_FCIC.md`. | 2-card: Vehicle(Options+Search), Person(DL+DH OOS-only), Boat(Options+Search). Devdoc combo order + EXISTENCE-ONLY routing conditions (State NOT_EXISTS / OLN NOT_EXISTS / RelatedHit NOT_EXISTS) for first-match + pool isolation; ALL value-comparison conditions removed (poisoned-array rule, QIDM_REFERENCE Sec 2a); DH KQ out-of-state only; DH+Boat destination state = NCIC dropdown; Boat QB stolen routing via relatedHitSearchIndicator in set[]; identifier-priority guardrails complete (Plate>VIN, OLN>Name, Hull>Reg); Attention auto-populated (handler); RMS Vehicle stripped to 3 attrs |
| TX_TLETS_CCH | providers/TX_TLETS_CCH/ | v1.1 | 113P/0F/0W/0LIM -- STUB: 14 QIDMs (6 base + 8 CCH). v1.1 (2026-07-15) base-6 QIDMs rebuilt to be identical to TX_TLETS v4.0 main (design rule: same except CCH addition) -- PascalCase, poisoned-array-free DL/DH, identifier-priority guardrails (Plate>VIN/OLN>Name/Hull>Reg), CAD combo defaults, Attention automation, QV-VIN CHECK-16 fix; Boat keeps main's RegistrationState-in-set[] divergence (TX_TLETS_CCH_ACCEPTED_DIVERGENCES.txt). CCH QH fixed to the real metadata bare Name combo (SSN/MiscNumber optional, not forced). NOT live-tested | Separate CCH-gated provider. Base 6 QIDMs identical to TX_TLETS main. All 8 CCH transactions (AQ/AR/FQ/IQ/QH/QR/QWI/ZR) on Person, autoSelect=false (named-checkbox via queryLabel), every CCH field CCH-suffixed (full isolation, zero collision), 3 CCH cards. Synthetic keyRefs (QR/QWI/ZR) + Choice splits; QH now 4 metadata-exact combos. FreeText capped display. CCH response QRDM out of scope. NOT live-tested |
| TX_TLETS | providers/TX_TLETS/ | v4.0 | 81P/0F/0W/0LIM -- v4.0 (2026-07-09) REBUILD under current methodology: PascalCase USx fieldIds (+ -PascalCaseUsxFields on RMS); versioned root filename; cleared PENDING_UPDATES flags (VehicleMakeName QRDM RND-62365 + ParseCommsysName args -- shared-module fixes verified present in JSON); condensed FL-style UI + full CHECK-15 label-hint pass (all 5 entities; Person keeps the shared OPTIONS card, emailAddress exposed/untouched, handler later); exposed MessageKey (CPL/DWI/RDL) on DriverLicenseQuery (metadata field-authority; CPL any[]); CHECK-16 reachability fixes -- set[] does NOT gate firing, so added existence-only EXISTS gates to make all metadata combos reachable (VIN FRT EXISTS, QV-VIN RegionId EXISTS with RegionId in any[]/metadata-faithful, DL DQName SexCode EXISTS + QWName BirthDate EXISTS, Boat BQ RegistrationState EXISTS); DH image-variant split MERGED to 2 combos (set[] never actually gated it -- ImageIndicator=Y default now triggers Reason=C + Email, all in any[]); docs 4-category migration + tests/->logs/ + phases/ retired. 22 CommSys combos. TESTED v4.0 (2026-07-10): full automation pass all 5 entities, enforce 25P/0F/0W, 95/95 logs verified, picklists scoped; BLOCK-OUT DEFERRED pending EmailAddress auto-populate handler (separate team -> rebuild + full re-test when delivered). Full history: providers/TX_TLETS/docs/tracking/CHANGELOG_TX_TLETS.md. | 7 cards (Veh 1, Per 3 [Options+DL+DH], Gun 1, Art 1, Boat 1), 22 CommSys combos, 6 QIDMs, PascalCase, identifier-priority guardrails (Plate>VIN, OLN>Name DL+DH, Hull>Reg), CHECK-16 reachability EXISTS-gates, MessageKey on DL, DH merged (Image=Y triggers Reason+Email trio; email exposed, handler later), email shared OPTIONS card, CAD plate defaults (PlateYear/PlateType on REG/RQ), DH-suffix+one-directional queriesToDeselect, TX-specific (DPSI/REG/VIN+FRT), -SkipRace on RMS |
...
| NY_NYSPIN_EJUSTICE_SKILLTEST | providers/NY_NYSPIN_EJUSTICE_SKILLTEST/ | v1.0 | 74P/0F/0W/0LIM -- v1.0 (2026-07-15) initial standup, independently rebuilt from source materials (not derived from providers/NY_NYSPIN_EJUSTICE/) as a skill/procedure-test exercise. Simulator-verified only (test_commsys.ps1 + run_test_matrix.ps1, 46/46); NOT live-tested, NOT scheduled for USx Tenant import. | Scoped strictly to devdoc literal "Basic Queries Supported" (6 of 16 metadata transactions: ArticleSingleQuery, BoatQuery, DriverHistoryQuery, DriverLicenseQuery, GunQuery, VehicleRegistrationQuery -- CBI/WMPI/Expanded Transactions Supported out of scope). 6 QIDMs, 16 combos. PascalCase 22-token fields + DH-suffix; Vehicle 2 cards, Person 3 cards (Options+DL+DH), Firearm/Article 1 card, Boat 2 cards. Devdoc-order combo arrays + RegistrationState NOT_EXISTS gates; identifier-priority guardrails (Plate>VIN, OLN>Name, Hull>Reg); Choice-set OOS combos (RVEHOUT; DALH/DALL/DALHOUT/DALLOUT) per LIMITATION #36; DH autoSelect=false + one-directional queriesToDeselect (AP #23 pattern). |
