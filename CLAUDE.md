...
| FL_FCIC | providers/FL_FCIC/ | v4.0 | 102P/0F/0W/0LIM (BASE) 102P/0F/0W/0LIM (MC) NEW | 2-card Person (DL+DH), QB routing (FL-8), one-directional queriesToDeselect, RegistrationStateDH, Attention visible |
...
| TN_TIES | providers/TN_TIES/ | v1.4 | 80P/0F/0W/0LIM (BASE) 80P/0F/0W/0LIM (MC) NEW | 6 basic queries, 28 combos, no State initialValue, MC multi-card (14 cards), DH-suffix |
...
## RMS Bundle — Clean Build from HIDLE_MC.json

**MC builds**: Load RMS from `templates/HIDLE_MC.json`. All standard patches (registrationState, autoSelect, camelCase fieldIds, licensePlateNumber) are pre-configured. Only dead field cleanup remains.

**BASE builds**: Load RMS from `source/HIDLE.json`. Apply standard patches (1, 3, 6, 7) as before.

**HIDLE_MC.json** is generated by `tools/generate_hidle_mc.ps1` from HIDLE.json with:
- All sourceField/combo values camelCase (licensePlateNumber, not LicensePlateNumber)
- LicensePlateNumberIn → licensePlateNumber everywhere (including QRDM/Results Layout)
- registrationState attribute on both Vehicle and Person QIDMs
- autoSelect=true on both QIDMs

**Dead field cleanup (all MC builds)**: Remove unused HIDLE fields per provider:
- Vehicle: `LicensePlateNumberOut`, `RegistrationStateOut`, `OwnerFirstName`, `OwnerLastName` + their combos
- Person: `socialSecurityNumber` (except AZ/TN), OOS-suffixed attrs + combos, optionally `race`
- See each provider's build script for exact dead field arrays

**No post-build patches.** If a new issue is found, update the build script or HIDLE_MC.json template — never add a JSON patch.
