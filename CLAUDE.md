...
| NY_NYSPIN_EJUSTICE | providers/NY_NYSPIN_EJUSTICE/ | v4.4 | 81P/0F/0W/0LIM -- v4.4 (2026-07-07) layout: DL NAME SEARCH (DGRP) card moved to the BOTTOM of the Person form (Options+DL+DH+DL-Name-Search) -- pure card-order change, no routing/entity impact. v4.3 restored MessageContinueKeyCode (a real DGRP metadata field wrongly dropped in v4.2 on a devdoc description -- metadata is the field-authority, not the devdoc); Age relabel "DOB Search Range (+/- years)". DGRP = full 10-field metadata set on its own card, autoSelect=TRUE (own field pool removes the v2.8 shared-Name co-fire cause, LIVE-VERIFY isolation). v4.1 removed genuine over-inclusions Gun ImageIndicator + Boat RelatedHitSearchIndicator (NOT in metadata). v4.3 live capture 73P/0F. Full re-test from T1 on re-import. Full history: `providers/NY_NYSPIN_EJUSTICE/docs/CHANGELOG_NY_NYSPIN_EJUSTICE.md`. | PascalCase, 9 cards (Veh 2, Per 4 [Options+DL+DH+DL-Name-Search], Gun 1, Art 1, Boat 2), 17 combos, 7 QIDMs, DGRP own card (last)+full 10-field metadata set (autoSelect), DH OOS combos (DALHOUT/DALLOUT) with State EXISTS/NOT_EXISTS routing, Vehicle plate OOS combo (RVEHOUT), NyNyspinTransactionName visible on DH (default DALL), PurposeCode default C on DH OOS combos, Choice-set OOS pattern (LIMIT #36), DH-suffix+one-directional queriesToDeselect, CAD defaults, State no-default (LIMIT #30), identifier-priority guardrails (Plate>VIN, OLN>Name DL+DH, Hull>Reg) |
