...
## Provider Status (updated 2026-07-16)
...
| HI_HCJDC_OFML | providers/HI_HCJDC_OFML/ | v4.8 | 65P/0F/0W/0LIM -- v4.8 (2026-07-15) removed QW shadow combo from DriverLicenseQuery (platform-auto-sent, not client-buildable -- FL_FCIC v4.2 precedent; DriverLicenseQuery now DQ+DQN only, matches devdoc exactly) + Person card label/layout pass (State label matches Vehicle's "leave blank for Hawaii"; M.I./Suffix/DOB regrouped; Sex label now "required with Name" since QW removal). Re-opened ALL 5 entities for live re-test (.test_state.json = authority). Full history: `providers/HI_HCJDC_OFML/docs/tracking/CHANGELOG_HI_HCJDC_OFML.md`. | 6 basic queries (Article/Boat/DH/DL/Gun/VehReg), 16 combos all reachable; single JSON; Person 3 cards (Search Options[State] + Driver License + Driver History, DH-suffix, one-directional deselect); Vehicle/Boat/Firearm/Article 1 card; Type Code dropdown (VEHICLE_TYPE/HI_NIBRS); ImageIndicator=N combo defaults on all VehReg combos; State in DL/DH any[] + VehReg any[] for OOS; identifier-priority guardrails complete (Plate>VIN, OLN>Name DL+DH, Hull>Reg); Name Last-first (v4.0) |
...
| FL_FCIC | providers/FL_FCIC/ | v7.3 | 92P/0F/0W/0LIM -- 31/31 combos, 6 QIDMs. v7.3 (2026-07-16) Firearm card cosmetic pass (DEX-1280, Gordon Hallof UX feedback): card title -> "FIREARM Query by Serial Number, "OR" NCIC Number, "OR" PCN"; GunSerialNumber "Serial Number" (dropped "(or use NCIC#/PCN)", card title now carries that context); GunMake -> "Gun Make (incl w/Serial Num only - optional)"; ImageIndicator -> "NCIC Image - if available". Label-only, no combo/QIDM change. Firearm re-opened for live re-test; Vehicle/Article/Boat preserved blocked at v7.1, Person unaffected (already open/PENDING from v7.2). Prior -- v7.2 (2026-07-16) Person DH card cosmetic pass (DEX-1278): card title -> "Driver History (Out-of-State) By Name "OR" Drivers License Number"; dropped "(DH)"/"required with Name" qualifiers from all 6 DH field labels now that the card title itself disambiguates (verify_build CHECK 15 Rule 2 downgraded FAIL->Info, portfolio-wide); reordered ROW_DH2 to First/Last/MI/DOB/Sex + added visible-only nameMiddleDH ("MI", unwired). Person re-opened for live re-test; Vehicle/Article/Boat/Firearm preserved blocked at v7.1 (unchanged fingerprints) at that time. Prior -- v7.1 (2026-06-30) Boat Hull>Reg guardrail completed for the FBQ in-state family (last of the Hull/Reg union over-send fixes; QB/BQ families fixed v6.9). Zero-error live pass certified (DEX-971). Full history: `providers/FL_FCIC/docs/tracking/CHANGELOG_FL_FCIC.md`. | 2-card: Vehicle(Options+Search), Person(DL+DH OOS-only), Boat(Options+Search). Devdoc combo order + EXISTENCE-ONLY routing conditions (State NOT_EXISTS / OLN NOT_EXISTS / RelatedHit NOT_EXISTS) for first-match + pool isolation; ALL value-comparison conditions removed (poisoned-array rule, QIDM_REFERENCE Sec 2a); DH KQ out-of-state only; DH+Boat destination state = NCIC dropdown; Boat QB stolen routing via relatedHitSearchIndicator in set[]; identifier-priority guardrails complete (Plate>VIN, OLN>Name, Hull>Reg); Attention auto-populated (handler); RMS Vehicle stripped to 3 attrs |
