...
## Provider Status (updated 2026-09-18)
...
| SC_SLED | providers/SC_SLED/ | v1.18 | 75P/0F/0W/2LIM | PARTIAL (36 logs, 5 owed) | **FIRST GROUND-UP PROVIDER SINCE THE TOOLCHAIN CHANGED** (2026-09-14) and the first build authored with the `Build-Qidm`/`Build-QidmAttribute`/`Build-QidmCombo` helpers rather than hand-written literals. 9 QIDMs / 17 combos / **5 QIFs — ONE CARD PER ENTITY** (v1.16; this cell said "18 combos / 6 QIFs" and described a 3-card Person and a 6th Administrative Message tab, all three retired between v1.13 and v1.16). **Three SC keys are COMPOUND and their MessageKey descriptions say so** — `QVRQ` "SC Vehicle Stolen/Reg", `QWDQ` "SC Driver Wanted/Reg", `QBBQ` "SC Boat Stolen/Reg" — against plain NCIC `QV`/`QWA`/`DQ`/`QA`/`QG`. The devdoc declares the mining outright: *"NCIC (QA, QB, QG, QV, QW) and DMV (Person and Vehicle) Tags returned from Data mining"*. ⚠️ **SC_SLED is the ONLY provider in the portfolio that builds a standalone `VehicleStolenQuery`** (FL/HI/NJ each removed theirs; MD never had one) — Rob's call 2026-09-14: *"build both and we can sort it out."* **MEASURED CO-FIRE, not shadowing:** `QV.P` set`[Plate]` equals `QVRQ.P` once PlateType=`PC`/PlateYear are form-prefilled, so one plate fires BOTH QIDMs and the stolen check goes out twice (the AZ `DQPN`/`DQP` exact-collision shape, which ordering cannot separate). Same on Person: `QWDQ` co-fires with `QWA.N`. ⚠️ **Masking stolen is NOT available yet and both mechanisms are disqualified** — `queriesToDeselect` alone is refuted for this shape (NY v2.8: the lower-threshold query sent twice, the higher zero), and `autoSelect=$false` has ZERO tenant-proven carriers (only TX_TLETS_CCH's 8, on a PARKED never-tested provider; its one tenant-observed outcome is the CA_eSUN failure where Send stayed DISABLED). **The first import is the discriminating test.** ⚠️ **EVERY ENTITY IS NOW A SINGLE CARD AND THE CO-FIRE IS THE DESIGN, NOT A SIDE EFFECT (v1.16, Rob: *"i want to keep it one card if possible"*).** `WantedPersonQuery` is TWO configs sharing ONE `query` — Person (`QWA.NCIC`/`OCA`/`N`) and Vehicle (`QWA.P`/`VM`, config name `_Veh`) — because **query checkboxes are keyed by ENTITY** (measured, `CHECKBOX_PROBE`) so a Vehicle-tab fill can never trigger a Person-entity query, while the wire carries `query` as `<MessageType>` so an invented name would ship to SC. That the platform keys on `(query, entity)` and not on `query` alone was PROVEN with `SHAREDQ_PROBE` before it was built. Result: **Person co-fires 3 searches, Vehicle co-fires 3** off one fill, nothing is duplicated on the form (the pool is per ENTITY, LIMITATION #26 — that sharing IS the co-fire), and the co-fire is stated in `test_commsys`'s CO-FIRE SUMMARY, the test plan's `coFire` map and the officer-guide banner rather than in a card title. The v1.5 DR-suffixed isolation card and the v1.15 second Wanted Person cards are all gone. `DriverRegistrationQuery`'s OLN branch is an authority-level DUPLICATE of DriverLicenseQuery's `DQ` (metadata keys both `DQ`, identical `<Requirements>`); its only unique contribution is the DQ **name** path. `DQ` is shadowed by `QWDQ` on a full name fill and reachable ONLY on an OLN-without-name fill. An **OLN-only search gets no wanted check** — SC's design, not a gap (`QWDQ` needs the full name triple; `QWA` defines no OLN-mandatory branch). `AdministrativeMessage` is **NOT BUILT** — removed at v1.13 by directive and registered `devdoc-combo-unbuilt`. It was the 7th form on a 6-entity platform, and a 7th form must DOUBLE an entity, which strands a permanently-dead greyed checkbox on both tabs of the doubled pair. Every removal lever was measured and failed the same day (`enabled:false` and `order` are not carried by our importer, a second provider bundle does not scope, `autoSelect:false` only pre-deselects, form↔interface unlinking did nothing), so six clean tabs minus one form was the trade. Restoring it means `targetEntity=Other` — the SIXTH entity, read out of the shipping frontend enum and confirmed live. Boat is a `<Choice>` nested under `<Set>` (hull\|reg) = ONE declared combination, TWO alternatives → synthetic `QBBQ.H`/`QBBQ.R` | [changelog](providers/SC_SLED/docs/tracking/CHANGELOG_SC_SLED.md) |
...
Full reference: `knowledge-base/PLATFORM_CONSTRAINTS.txt` (22 APs + 32 LIMITATIONs + 3 CAPABILITIES, non-contiguous AP #2-#27 / LIMITATION #1-#43 + #45-#46 / CAPABILITY #44 + #47 + #48, with cross-reference index).
...
## Tools (129 scripts + 23 shared modules in `tools/`, + `tools/_probes/` (17 graduated ad-hoc probes -- this count has gone stale twice, so verify it by `ls` before citing -- incl. `probe_qidm_helpers.ps1` -- LAW 2 for the QIDM build helpers, incl. `probe_pull_verdict.ps1` -- LAW 2 for the config-pull verdict, composed into `doctor`: it EXTRACTS the three verdict functions from `ui.js` and runs 8 cases in headless Edge, because both pull buttons used to paint a run that pulled NOTHING green with a tick, incl. `probe_multi_qif_entities.ps1` -- the DENOMINATOR for any change to per-entity form handling: which providers put >1 QUERYINPUTFORM on one entity (measured 2026-09-16: **1 of 21, SC_SLED only**), which is what turned "the other 20 cannot be affected" into a measurement when `emit_test_plan`'s hashtable QIF lookup was found EVICTING forms, incl. `probe_control_types.ps1` -- **WHAT FORM CONTROLS THE PLATFORM ACTUALLY HAS**, censused across our 21 JSONs *and* all 66 tenant configs including NOT-OUR-BUILD (so it is evidence about the PLATFORM, not our habits): `FormInput`/`FormSelect`/`FormDate`/`FormCheckbox`/`FormRadioGroup` and nothing else -- **no textarea/multi-line control exists**, and a `FormInput` has only ever carried `fieldId`/`label`/`maxLength`/`initialValue`. Run it before promising any new control: an unknown `resolvedName` renders NOTHING silently, incl. `reconcile_ledger_vs_reality.ps1` -- the records-vs-deployed table, asked for twice -- and `audit_auth_uniformity.ps1`, the RND-71625 AUTHENTICATION-config comparison against CA_CLETS as reference, with an in-memory self-test that refuses to report UNIFORM unless a planted difference is provably detected) + `tools/config/` (9 JSON reference tables, incl. `tenant_scope.json` -- WHICH TENANTS ARE IN SCOPE, exclusion-with-a-reason, ships EMPTY and its `_candidates` block is a proposal only -- and `tenant_map.json` -- the canonical subdomain <-> deptId <-> ledger-name join) + 3 archived tools in `tools/_archive/`)
...
| `pipeline.ps1` | **ONE-COMMAND PIPELINE** -- build + report + metadata + sync + version docs + cross-provider + repo audit + enforce in 8 steps; stops on first failure. **RE-CUTS `providers\IMPORT_JOB.json` ON A VERSION BUMP (added 2026-09-18)** — Rob: *"recut the job  that should be automatic"*, after the panel twice told him v1.11 while the repo was on v1.12. A bump already re-emits the test plan, spec plan and picklist scope; the import job was the odd one out, and a STALE job is worse than none because the panel states its version confidently. **The tenant is RESOLVED, never derived from the name** — `usx-<provider>` is not a reliable transform (NM_NMLETS_OFML lives on `usx-nm-nmlets`), so it asks `Resolve-TenantIntent`, the same authority `serve_plans /target` and `emit_import_job` use, which tenant records THIS provider as its intent. **Two guards, both proven by test:** it fires ONLY on a version change, and it **refuses to clobber an unfinished job for a different provider** (IMPORT_JOB.json is ONE file — rebuilding X would otherwise discard a job staged for Y, e.g. the Newark job sitting one click from done in `HELD_TENANT_WORK.md`). 0 or >1 matching tenants → prints a NOTE and cuts nothing; guessing which tenant to authorise an import against is the class of error the deploy guards exist to stop | `-Provider <name>` (required) `-SkipBuild` `-SkipEnforce` |
...
| `audit_query_selectable.ps1` | Can the officer actually SELECT and send every built query — `autoSelect=$false` can render a checkbox that never activates, leaving Send disabled while every other gate reads green. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Provider <name>` `-Path <json>` `-All` `-Quiet` `-OutFile` |
...
| `audit_extension_syntax.ps1` | Extension JS syntax gate for `automation/extension/*.js` (composed into `doctor`) — a parse error is total and silently killed the driver/capture tools for five days in 2026-09. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Path <dir>` `-OutFile` `-Quiet` |
| `audit_extension_version.ps1` | **A CHANGED EXTENSION MUST ANNOUNCE A NEW VERSION** (pre-commit + `doctor`; it was missing from this table until 2026-09-18, which is its own hole). The extension is reloaded BY HAND, so a stale one and a broken one look identical unless the version moves. ⚠️ **ROB HAS HAD TO SAY THIS THREE TIMES AND THE GATE WAS WRONG EACH TIME — it is the LAW 2 failure occurring inside the gate named after the rule.** 09-16 *"advance the extnsion version every time you make a change"* → built, watching ui.js's banner only. 09-17 *"the extension is not advancing the version like i asked"* → driver.js had changed twice while its own banner sat three weeks behind, so a per-file banner check was added. 09-18 *"still no version bump on extension   this is getting tiring havig to rpeat what are global instructions"* → **`manifest.json` still read `0.8.0`, untouched for weeks, while the gate reported PASS on a bump of three console strings.** The manifest version is what the BROWSER shows and what "the extension version" means to an operator; a console banner is an in-page announcement we invented. Both are now gated, and the manifest check runs FIRST so no early PASS can skip it, keyed on **any** changed file under `automation/extension/` (not just `.js`). `-Bump` advances all four: ui.js + every bannered script (same stamp — per-file drift is how driver.js fell behind) + manifest patch. Proven failable 2026-09-18 with a planted change | `-Staged` `-Bump` `-OutFile` `-Quiet` |
| `audit_deploy_guards.ps1` | Proves the only tenant-write path (`deploy_probe.js`) actually refuses on every planted fault, via mutation against a real DOM (composed into `doctor.ps1`). See `knowledge-base/TOOL_REFERENCE.txt`. | `-OutFile` `-Quiet` |
...
| `emit_import_job.ps1` | Writes the reviewed `providers\IMPORT_JOB.json` a human reads before the tenant console executes it — plans on content hash, never a label, one tenant per call. See `knowledge-base/TOOL_REFERENCE.txt`. | `-DeptId <id[,id]>` `-Provider <NAME>` `-All` `-DryRunOnly` `-Force` `-TenantDir` `-OutFile` `-Quiet` |
| `emit_pull_job.ps1` | **THE READ-SIDE JOB FILE** — writes `providers\PULL_JOB.json` (served at `GET /pulljob`, run by the panel's one visible **RUN THE JOB** button), so the config-pull tenant set is a reviewable artifact instead of an 800-char paste. Every tenant carries a `why` (`baseline` = UNCHANGED is the good outcome / `never-pulled` / `new-tenant`) because those have OPPOSITE success conditions. ⚠️ **Scoping the default set to the config inventory OMITS THE TENANT YOU MOST WANT** — the inventory is what was already pulled, so `dallastx-foundation` was missing from the first draft; `tenant_map.json` is the authority for "ours". ⚠️ **`firstSeen` CANNOT detect a new tenant** (the roster was bootstrapped in one pass, so all 1,788 rows share it) — hence explicit `-Include`, since a detector that silently returns nothing is worse than a parameter. Refuses: empty job, missing/0-tenant roster, DEACTIVATED and `tenant_scope` exclusions (listed in `skipped` **with a reason**). See `knowledge-base/TOOL_REFERENCE.txt`. | `-DeptId <id[,id]>` `-Include <id[,id]>` `-BaselineOnly` `-MaxAgeDays <n>` `-RosterPath` `-OutFile` `-Quiet` |
| `report_import_plan.ps1` | Read-only: what would an import actually change, judged by bundle content hash (never a label) — refuses NOT-OUR-BUILD configs and label-only diffs. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Provider <name>` `-TenantDir <dir>` `-IncludeDeactivated` `-OutFile` `-Quiet` |
| `verify_tenant_import.ps1` | Did the import actually land? Three-way BEFORE/AFTER/REPO content-hash comparison — UNPROVEN without a captured BEFORE, never inferred from a label. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Tenant <subdomain\|deptId>` `-ExpectedProvider` `-ExpectedVersion` `-TenantDir <replica>` `-OutFile` `-Quiet` |
| `propose_ledger_patch.ps1` | Proposes `IMPORT_LEDGER.md` corrections from content-hash evidence; never writes the ledger — Rob stays the author. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Tenant` `-TenantDir` `-LedgerPath <replica>` `-OutFile` `-Quiet` |
...
| `refresh_tenant_reports.ps1` | Regenerates every derived tenant report from current data (and names the ones needing a fresh browser pull instead) — verified by write time, never `Test-Path`. See `knowledge-base/TOOL_REFERENCE.txt`. | `-CheckOnly` `-Quiet` |
| `identify_notours_provider.ps1` | Groups NOT-OUR-BUILD tenant configs by which USx provider would have to support each one, and flags byte-identical families vs one-offs. See `knowledge-base/TOOL_REFERENCE.txt`. | `-TenantDir` `-OutFile` `-Quiet` |
| `report_notours_reuse.ps1` | Of the configs we didn't build, which are copied across many tenants vs genuine one-offs — compared by provider-bundle hash, not tenant count. See `knowledge-base/TOOL_REFERENCE.txt`. | `-TenantDir` `-OutFile` `-Quiet` |
| `report_tenant_groups.ps1` | Every discovered tenant grouped for a human decision (ours/not-ours measured by hash+stamp; real-vs-dev-test is a stated heuristic) — HTML + PDF. See `knowledge-base/TOOL_REFERENCE.txt`. | `-TenantDir` `-OutFile` `-PdfFile` `-IncludeDeactivated` `-Quiet` |
...
| `build_translate_test.ps1` | **DOES THE PLATFORM TRANSLATE A CODE ON THE WAY OUT?** Generates `providers\TRANSLATE_TEST.json` — a throwaway rig for NY's real requirement (Rob 2026-09-15): the officer picks a NY numeric in-state plate type and **many** of those must collapse to **one** NCIC code out-of-state. ⚠️ **LIMITATION #38 already says there is NO configurable mapping layer** — the wire value IS the selected attribute's code, and it names this very field (`LicensePlateTypeCode "PC - REGULAR…"→PC`). So the rig tests the only two mechanisms still plausible, plus a control: **M1** a combo `defaults[]` on a field the officer ALREADY filled (does it OVERRIDE or only fill-when-empty? we ship these everywhere and have never measured it — `audit_wiring_closure` only checks a default is *reachable*), **M2** attribute-level `codeTypeProvider` DISAGREEING with its control (already emitted by `Build-QidmAttribute`, but attribute and control have always agreed), **C** a no-rule/no-default attribute so "tag missing" and "translated to empty" can't be confused. Two submits, one import, three answers. ⚠️ **Prove the observation channel FIRST** (any query on the target tenant must show an outgoing XML in dex-log) and note an import **REPLACES** the bundle set. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Provider <name>` `-OutPath` `-InStateValue` `-OutOfStateCode` |
| `build_sharedquery_probe.ps1` | **CAN ONE `query` LIVE ON TWO ENTITIES AT ONCE?** Built 2026-09-18 for Rob's restructure — *"the wanted person query will need to cofire with person queires and vehicle queires were appropriate ... roll the wanted person query onto to each tab"*. Splitting one transaction across two tabs is boxed in by two MEASURED facts: checkboxes are keyed by **entity** (so Person and Vehicle combos must be separate QIDMs — a Vehicle-tab fill can never trigger a Person-entity query), and **the wire carries `query` as `<MessageType>`** (read from a real capture), so both QIDMs must share the same `query` — an invented name would ship on the wire and be rejected. That forces same `query` + different `name` + different `targetEntity`. ⚠️ **THE UNKNOWN IT TESTS:** the platform spec says `query` is *"used to match this config with the form the user submitted"* and never says whether the match is by query ALONE or `(query, entity)`. If query alone, one config is **silently dropped with every structural gate green** — the exact failure class this repo exists to catch. Carries a CONTROL QIDM per entity with a unique query name so a null result is readable (control renders + shared doesn't = the shared query is the cause; neither renders = probe broken, conclude nothing). ⚠️ THROWAWAY RIG, HAND-IMPORT ONLY | `-OutPath` |
| `build_checkbox_probe.ps1` | **WHAT CREATES, AND WHAT REMOVES, A QUERY CHECKBOX?** Built 2026-09-18 after Rob asked the two questions that exposed the gap — *"did we probe the checkbox creation mechnism  were you gusing at naming values"*. Answers: **NO, we had not probed it** (every claim came from READING the captured federatedSearch bundle — static analysis, never measured against a config we controlled), and the names are **read, not invented** (`enabled`/`order`/`queryableEntityType` are literal identifiers in that bundle; `Other` came from the enum `ut` and was CONFIRMED LIVE at 6 tabs). The unproven link is whether the **IMPORTER carries `enabled`/`order`** from our JSON into the model the client reads — a client consuming a field says nothing about the config pipeline emitting it. Four questions in ONE import, all on entity `Other` so no real entity is disturbed: **Q1** two forms share one entity and `CBControl` needs a field only form A has — does its checkbox appear on BOTH tabs (the per-entity rule, and the CONTROL without which a null result means nothing)? **Q2** does `enabled:false` remove it everywhere (= the removal lever) or does the importer strip the field? **Q3** does `autoSelect:false` still RENDER, merely unticked — the distinction this repo has conflated everywhere, including SC_SLED's stolen-query note? **Q4** does a SECOND PROVIDER BUNDLE scope a tab (the admin UI has *"Link forms to interface — the following query forms can be sent to this interface"*, so linkage exists somewhere; if provider identity alone scopes, 7 clean tabs is pure config). Also emits `order` on both forms — if the tabs flip, SC_SLED's host-entity juggling for tab placement can be retired. ⚠️ THROWAWAY RIG, HAND-IMPORT ONLY, and an import REPLACES the bundle set | `-Entity <name>` `-OutPath` |
| `build_multiline_test.ps1` | **CAN ANY FORM CONTROL SHOW MORE THAN ONE LINE? — YES, AND THIS RIG IS HOW WE KNOW.** ✅ **ANSWERED 2026-09-17 on the first import: `FormTextarea` — LOWERCASE `a` — RESOLVES and renders a wrapping, auto-growing, scrollable multi-row box. `FormTextArea` (capital A, the spelling anyone would write by hand) does NOTHING — the resolver is EXACT-MATCH, and the validator's duplicate-tag refusal is the only reason both casings actually got tested. All 5 candidate PROPS dead; all 7 tabs rendered, so AN UNKNOWN `resolvedName` IS IGNORED, NOT FATAL (HALF B's stated risk, now measured away); `Text` did NOT render, so there is still no help-text channel. ⚠️ TRANSMISSION IS NOT PROVEN — the rig wires only the COMPANION, so that box sends nothing, and whether a NEWLINE survives the wire is open. Full record: PLATFORM_CONSTRAINTS `CAPABILITY #48`.** Throwaway rig (`providers\MULTILINE_TEST.json`, not a provider) built because the then-**LIMITATION #48** said no but three of its four sources are USAGE CENSUSES — the Forge migration guide says so in its own words — and the fourth capability doc is from February. Two halves, deliberately separated by RISK: **HALF A is zero-risk** — 5 candidate *props* (`multiline`/`rows`/`type=textarea`/`numberOfLines`/`maxRows`) on known-good `FormInput`s beside a baseline on the CONTROL tab, since an unrecognised prop is ignored and any box taller than the baseline is the answer. **HALF B is not** — 6 candidate `resolvedName`s, ONE FORM EACH, because an unresolved component may THROW while deserialising the Craft.js tree rather than degrade quietly (unlike an unknown `targetEntity`, which SC_SLED v1.0 measured as harmless). Every candidate form carries a known-good COMPANION input so *both boxes* = resolves, *companion only* = ignored, *tab missing* = fatal to that form, *no tabs* = fatal module-wide. Includes `Text`, which provably exists in `QUERYRESULTSLAYOUT` — if it renders in a QIF we gain a **help-text channel the capability doc says is impossible**, worth more than the textarea. ⚠️ **The validator caught the first build as an INERT TEST** — name-derived tags collided between `FormTextArea`/`FormTextarea` (case-insensitive duplicate → silent overwrite), so it would have tested one casing while reporting two; tags are index-prefixed now. ⚠️ **HAND-IMPORT ONLY:** the bridge refuses to serve it and the DEPLOY guards refuse it because `/target` resolves the tenant's intent as a real provider — both working as designed. An import REPLACES the bundle set, so put the real build back after. | `-Components <list>` `-OutPath` |
...
| `emit_picklist_scope.ps1` | Emits the PICKLIST_SCOPE.json the browser scope tool consumes. **Buckets by TAB (the owning QUERYINPUTFORM), not by `targetEntity` — corrected 2026-09-18.** Tabs are keyed by QIF (CAPABILITY #47), so SC_SLED's `ENTITY_WantedPerson` (targetEntity=`Firearm`) had its 5 dropdowns filed under "Firearm": the panel offered a "Wanted Person" tab the scope had no key for, the button silently did nothing, and scoping "Firearm" instead downloaded 5 `field not found in DOM` rows that then sat in `TENANT_PICKLISTS.json` looking like evidence. The dedup key was `entity\|fieldId` too, so a control on BOTH forms of a doubled-up entity was dropped with no message. `tab` is derived exactly as `emit_test_plan` derives it, so the two cannot drift; a single-QIF entity gives tab == entity, which is why the other 20 providers are unaffected. Re-run on every build by `reset_test_package` 6c | `-Path <json>` |
...
| `ingest_tenant_roster.ps1` | Diffs the one-page department index against the committed baseline to find new/renamed/status-changed tenants, without a full 1,785-page census. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Path <file>` `-Update` `-MaxShrinkPct <n>` `-OutFile` `-Quiet` |
...
| `ingest_tenant_versions.ps1` | Three-way compare per tenant — MEASURED export vs LEDGER claim vs REPO version — from the extension's per-tenant Export JSON pull. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Path <dir>` `-OutFile` `-Quiet` |
| `ingest_tenant_configs.ps1` | The baseline record of what's deployed where — extracts every tenant's full config + canonical SHA-256 to the gitignored `_versions\tenant_exports\`. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Path <dir>` `-OutFile` `-Quiet` |
| `audit_tenant.ps1` | One tenant, everything we know, on demand (identity, ledger claim, deployed config, content provenance) — MANUAL-ONLY, keyed by subdomain or deptId. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Tenant <subdomain\|deptId>` `-OutFile` `-Quiet` |
| `audit_tenant_provenance.ps1` | Identifies which historical build a tenant bundle actually is, by content hash against every version in git — not by its (often stale) label. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Providers <list>` `-TenantDir <dir>` `-Rebuild` `-IncludeLegacy` `-OutFile` `-Quiet` |
| `get_provider_version.ps1` | Retrieves any prior provider JSON byte-exact from git history — retrieval, not rebuild; an old build script does not reproduce an old version. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Provider <name>` `-List` `-Version <X.Y>` `-IncludeLegacy` `-Commit <sha>` `-Variant <legacy\|MC\|BASE>` `-All` `-OutPath <dir>` `-Force` |
...
| `serve_plans.ps1` | The localhost bridge the browser extension reads the repo through (test plans, deploy payloads, `/target/<deptId>` provider-intent resolution, `/roster`). Verify a running instance by CURLING its endpoints, never by process start time. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Port <n>` |
| `install_git_hooks.ps1` | Installs the pre-commit hook that blocks a commit breaking extension JS or `tools/*.ps1` — `.git\hooks` isn't version-controlled, so this must be re-run per clone. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Verify` |
| `watch_imports.ps1` | Waits for the post-import AFTER-export and runs `verify_tenant_import` automatically, closing the import→export→compare loop without a manual step. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Once` `-DeptId <id[,id]>` `-TimeoutSec` `-PollSec` `-Quiet` |
...
| **What anti-patterns apply?** | `knowledge-base/PLATFORM_CONSTRAINTS.txt` (22 APs + 32 LIMITATIONs + 3 CAPABILITIES, non-contiguous AP #2-#27 / LIMITATION #1-#43 + #45-#46 / CAPABILITY #44 + #47 + #48) | Memory, training data |
...
**PHASE 2 STEP [2b] TENANT PICKLIST COVERAGE -- BLOCKING, added 2026-09-18.** Rob: *"too many holes
in this process you need to tighen it up   the tests get updated with every json build   pick ist is
part of the test process and needs prompting."* The SCOPE was already regenerated on every build
(`reset_test_package` 6c) but **nothing ever asked whether the capture was TAKEN** -- the only signal
was an advisory `[NOTE]` from `audit_picklist_scope` at the tail of a 742-line `enforce` run, which is
a footnote, not a prompt. SC_SLED was built, swept for two days and imported with **zero dropdowns
captured** while every board read green. It **BLOCKS** rather than warns because an uncaptured dropdown
means the driven test values were never checked against the options the TENANT renders -- the same
class as FILLABILITY (a test that cannot fire), except it fails by *selecting nothing*, which reads as
a provider defect rather than a data-prep one. Reported **per TAB**, never per entity, and a record
carrying an `error` or `count: 0` is **NOT** counted as coverage -- counting it would have turned
SC_SLED's five `field not found in DOM` rows into evidence of the very gap they prove. LAW 2 both ways:
FAILs on SC_SLED (Wanted Person 0/5) and CA_VENTURA_COUNTY (5 tabs), PASSes on TN_TIES and OR_LEDS.
