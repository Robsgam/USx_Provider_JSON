...
## Provider Status (updated 2026-07-23)
...
| AZ_AZDPS | providers/AZ_AZDPS/ | v3.0 | 72P/0F/0W/0LIM -- v3.0 (2026-07-22) METHODOLOGY REBUILD (consolidated the legacy BASE+MC dual-script/dual-JSON into ONE versioned single-JSON build; native PascalCase USx fields via -PascalCaseUsxFields; docs migrated to the 4-category structure; phases/ retired; GunQuery serialNumber CAD fix ported from the NJ/FL/HI/TX class; cleared the RND-62365 VehicleMakeName-QRDM + PARSECOMMSYS-ARGS pending flags by rebuilding on the current shared modules). NOT yet USx-tenant-tested at v3.0. | dexStateUserId hidden badge, DH-suffix isolation, WMPI Wanted/Missing queries, RegistrationStateDH hidden SelH (initialValue AZ), Attention auto-handler (CommsysGetLastNameFirstNameInitialRuleHandler), KeepSsn, invented keyRefs (ACVRV/DQN/KQH/ACQWN/ACQMN), 8 QIDMs / 18 combos |
