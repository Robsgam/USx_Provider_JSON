...
| FL_FCIC | providers/FL_FCIC/ | v7.5 | 92P/0F/0W/0LIM -- 30/30 combos, 6 QIDMs. v7.5 (2026-07-16, direct feedback, no DEX ticket): Person CARD_DL ROW_DL2 Name order fixed to First/Last/Middle to match CARD_DH's v7.2 order (was Last/First/Middle, inconsistent). Vehicle collapsed from 2 cards to 1 -- merged the "Search Options" card into the Search card, State + NCIC Image moved to the bottom row alongside Decal Number. Both label/layout-only, no combo/QIDM change. Person + Vehicle re-opened for live re-test; Firearm stays open/PENDING at v7.3; Article/Boat preserved blocked at v7.1. Prior -- v7.1-v7.4: Boat Hull>Reg guardrail completion (v7.1, DEX-971 zero-error pass) + the DEX-1278/1279/1280 Person/Firearm/Vehicle card-by-card cosmetic pass (card-title-carries-disambiguation pattern; verify_build CHECK 15 Rule 2 DH-label gate downgraded FAIL->Info portfolio-wide + Rule 3 Purpose Code exemption; FRQTitleLienInformation combo intentionally removed, approved-skip, 31->30 combos). Full history: `providers/FL_FCIC/docs/tracking/CHANGELOG_FL_FCIC.md`. | 1-card Vehicle (Plate/VIN/Make/Year rows + Decal+State+Image bottom row, v7.5), Person(DL+DH OOS-only), Boat(Options+Search). Devdoc combo order + EXISTENCE-ONLY routing conditions (State NOT_EXISTS / OLN NOT_EXISTS / RelatedHit NOT_EXISTS) for first-match + pool isolation; ALL value-comparison conditions removed (poisoned-array rule, QIDM_REFERENCE Sec 2a); DH KQ out-of-state only; DH+Boat destination state = NCIC dropdown; Boat QB stolen routing via relatedHitSearchIndicator in set[]; identifier-priority guardrails complete (Plate>VIN, OLN>Name, Hull>Reg); Attention auto-populated (handler); RMS Vehicle stripped to 3 attrs |
