...
| CA_CONTRA_COSTA | providers/CA_CONTRA_COSTA/ | v2.0 | 77P/0F/0W/0LIM -- v2.0 (2026-07-23) SPECIAL-CASE framework build: CA_CLETS COPY (CA_CLETS metadata = template for the 6 basic-supported families). Derived metadata = CA_CLETS.xml + CC's 2 JAWS transactions merged (source/CA_CONTRA_COSTA.xml = 181 txns; CC original preserved as _JAWS_ONLY.xml). CC's JAWS (CaContraCostaJawsPersonQuery/WarrantQuery) + CLETSPersonSuperQuery are CC "Expanded Queries" -- carried in metadata but NOT built (CC devdoc "Basic Queries Supported: None"); backfill if promoted. NOT USx-tenant-tested. See memory project_ca_contra_costa_special_case. | CA_CLETS twin (6 families / 40 combos / PascalCase / -SkipRace); JAWS+SuperQuery unbuilt-in-metadata; RequestingAgencyId only on the (unbuilt) JAWS combos |
