...
**MC builds**: RMS bundle and CommSys QRDM are constructed by `tools/_build_rms_bundle.ps1` from inline KB specifications. No external template dependency (no HIDLE.json). Build scripts dot-source the module and call:
...
| **What does the RMS bundle contain?** | `tools/_build_rms_bundle.ps1` (MC) or CLAUDE.md RMS Bundle section + `knowledge-base/BUILD_RULES.txt` Section 4 (BASE) | HIDLE.json inspection alone |
