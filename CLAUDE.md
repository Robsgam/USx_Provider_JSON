...
| CA_CLETS | providers/CA_CLETS/ | v2.10 | 77P/0F/0W/0LIM -- v2.10 (2026-06-26): VehicleMakeName result-mapping code source corrected VEHICLE/VehicleType -> attributeType=VEHICLE_MAKE/codeTypeSource=NCIC (RND-62365; probe-confirmed present; matches RND-54190 + sibling VehicleModelName). Shared module; result-mapping only, request-side combos unchanged; re-import + full re-test from T1. v2.9 Attention auto-populate on DH (CommsysGetLastNameFirstNameInitialRuleHandler + hidden gate-feeder); serialNumber fieldId on Firearm (was GunSerialNumber) enables CAD-triggered IG.QGB; test_cad_dispatch.ps1 added (18 PASS / 0 FAIL / 7 SKIP). Person + Firearm re-open for testing. | purposeCode (CAD-aligned fieldId), State routing (blank=in-state via set[] presence, NOT conditions), DH-suffix fieldIds, cross-entity Name on Veh/Gun/Boat, no ImageIndicator, 6 basic queries, yyyyMMdd dates, CAD defaults on IA.QV. v2.7: OLN-wins mutual-exclusion conditions on DL combos (IR.QVC.O RegistrationState NOT_EXISTS; ID.L1 RegistrationState+criminalIdNumber NOT_EXISTS; IR.QVC.C/S OperatorLicenseNumber NOT_EXISTS); APPSRequestIndicator added (devdoc O field, any[] on IR.QVC.N + APPS Check form field); GunQuery Age+BirthDate added (IG.QGH any[] + Firearm form); ArticleCategory added (article combos any[] + Article form). v2.6: PascalCase all 22 USx CAD fieldIds (layout + sourceField + set[]/any[]), identifier-priority guardrails (Plate>VIN on NLTS.RQ.V+IA.QVK, OLN>Name DL on NLTS.DQ.N+IR.QVC.N+IN.L1, OLN>Name DH on NLTS.KQ.N, Hull>Reg on NLTS.BQ.R+IA.QB.R), -PascalCaseUsxFields on RMS. v2.5: removed 20 inert value-comparison conditions (poisoned-array). |
