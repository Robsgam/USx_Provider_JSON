...
| TX_TLETS | providers/TX_TLETS/ | v3.11 | 81P/0F/0W/1LIM -- v3.11: REVERTED Attention sourceField to @('Attention') (v3.10 emptied it; ConnectCic REJECTS empty sourceField at import -- live-proven HI v2.5; KB corrected; Attention auto-populate INERT on this platform, known limitation); re-import + full re-test from T1; v3.9: Attention auto-populated via CommsysGetLastNameFirstNameInitialRuleHandler (no visible field; automated-Attention standard, BUILD_RULES Sec 14); re-import + full re-test required, test pkg reset to T1; v3.8: restore 4-combo DH (image-variant split); Image=Y serializes only when email present (union-exclusion); plain combos fire name/OLN without email; re-import required; testing restarts T1 | 6 cards (Veh 1, Per 3 DH+DL+Options, Gun 1, Art 1, Boat 1), 24 combos, 6 QIDMs, DH image-variant split (KQNameImg/KQName/KQOLNImg/KQOLN), email→shared OPTIONS card, CPL Name combo, DH-suffix+one-directional queriesToDeselect, TX-specific (DPSI/REG/VIN+FRT), CAD defaults, -SkipRace on RMS |
