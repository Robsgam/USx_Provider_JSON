...
| TX_TLETS | providers/TX_TLETS/ | v3.12 | 83P/0F/0W/0LIM -- v3.12 (2026-06-23) IDENTIFIER-PRIORITY ROLLOUT + ATTENTION RESOLVED: all 3 guardrail pairs applied -- Plate>VIN (licensePlateNumber NOT_EXISTS on VIN/RQ/QV vehicle combos), OLN>Name DL (operatorLicenseNumber NOT_EXISTS on DQName/QWName/CPLName) + DH (operatorLicenseNumberDH NOT_EXISTS on KQNameImg/KQName), Hull>Reg (boatHullIdNumber NOT_EXISTS on QBRegistrationNumber). conditions[].field = camelCase QIF sourceField (live-proven HI/FL pattern, verify_build CHECK 13). ATTENTION now RESOLVED (reverses v3.11 INERT conclusion) via HI v2.9 pattern: 'Attention' in all 4 DH combo any[] + hidden InpH gate-feeder (initialValue='X') on DH card + sourceField=['Attention'] + Attention=X combo default (audit_cad CHECK 6); pending live confirm. vehicleYear added to VIN combo any[]. QBRegistrationNumber: boatHullIdNumber removed from any[] (contradicted its own NOT_EXISTS gate). 22 CommSys combos. re-import + full re-test from T1, test pkg reset to T1. v3.11: REVERTED Attention sourceField to @('Attention') (v3.10 emptied it; ConnectCic REJECTS empty sourceField at import); v3.9: Attention auto-populated via CommsysGetLastNameFirstNameInitialRuleHandler; v3.8: restore 4-combo DH (image-variant split); Image=Y serializes only when email present (union-exclusion) | 6 cards (Veh 1, Per 3 DH+DL+Options, Gun 1, Art 1, Boat 1), 22 CommSys combos, 6 QIDMs, identifier-priority guardrails (Plate>VIN, OLN>Name DL+DH, Hull>Reg), DH image-variant split (KQNameImg/KQName/KQOLNImg/KQOLN), email→shared OPTIONS card, CPL Name combo, Attention auto-populate (gate-feeder), DH-suffix+one-directional queriesToDeselect, TX-specific (DPSI/REG/VIN+FRT), CAD defaults, -SkipRace on RMS |
