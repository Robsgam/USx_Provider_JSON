...
## Provider Status (updated 2026-07-24)
...
`enforce.ps1` runs ~10 phase sections: PHASE 1 (build freshness), 2 (validator scores), 2b (metadata-divergence gate), 2c (structural verify + CAD gates), 2d (simulator parity), 2e (devdoc supported-query gate), 2f (reproducibility — opt-in via -Reproducible), 3 (doc version sync — 8 locations per provider: CLAUDE.md, STATUS, SQVR, JSON_INVENTORY, BUILD_NOTES + date checksum, REBUILD_TRACKER, per-provider CHANGELOG_<PROVIDER>.md, repo-root CHANGELOG.md Current line), 4+5 (cross-provider + repo integrity, run in parallel), 6 (iterate-phase gate + hypothesis quarantine), git status. Exit 0 = verified. Exit 1 = blocked.
