...
| TX_TLETS | providers/TX_TLETS/ | v3.7 | 81P/0F/0W/1LIM -- v3.7: emailAddress moved to set[] on both DH combos (structural enforcement; devdoc requires Email when Image=Y; CAD DH suppressed until email-injection handler); re-import required; DH tests restart T10 | 6 cards (Veh 1, Per 3 DH+DL+Options, Gun 1, Art 1, Boat 1), 22 combos, 6 QIDMs, v3.4 poisoned-array fix (merged Img/catchall pairs DL 7→4, DH 4→2), email→shared OPTIONS card, CPL Name combo, DH-suffix+one-directional queriesToDeselect, TX-specific (DPSI/REG/VIN+FRT), CAD defaults, -SkipRace on RMS |
