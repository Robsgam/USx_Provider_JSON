...
| FL_FCIC | providers/FL_FCIC/ | v7.2 | 92P/0F/0W/0LIM -- 31/31 combos, 6 QIDMs. v7.2 (2026-07-16) Person DH card cosmetic pass (DEX-1278, Gordon Hallof UX feedback): card title -> "Driver History (Out-of-State) By Name "OR" Drivers License Number"; dropped "(DH)"/"required with Name" qualifiers from all 6 DH field labels now that the card title itself disambiguates (verify_build CHECK 15 Rule 2 downgraded FAIL->Info, portfolio-wide); reordered ROW_DH2 to First/Last/MI/DOB/Sex + added visible-only nameMiddleDH ("MI", unwired). Label/layout only, no combo/QIDM change. Person re-opened for live re-test; Vehicle/Article/Boat/Firearm preserved blocked at v7.1 (unchanged fingerprints). Prior -- v7.1 (2026-06-30) Boat Hull>Reg guardrail completed for the FBQ in-state family (last of the Hull/Reg union over-send fixes; QB/BQ families fixed v6.9). Zero-error live pass certified (DEX-971). Full history: `providers/FL_FCIC/docs/tracking/CHANGELOG_FL_FCIC.md`. | 2-card: Vehicle(Options+Search), Person(DL+DH OOS-only), Boat(Options+Search). Devdoc combo order + EXISTENCE-ONLY routing conditions (State NOT_EXISTS / OLN NOT_EXISTS / RelatedHit NOT_EXISTS) for first-match + pool isolation; ALL value-comparison conditions removed (poisoned-array rule, QIDM_REFERENCE Sec 2a); DH KQ out-of-state only; DH+Boat destination state = NCIC dropdown; Boat QB stolen routing via relatedHitSearchIndicator in set[]; identifier-priority guardrails complete (Plate>VIN, OLN>Name, Hull>Reg); Attention auto-populated (handler); RMS Vehicle stripped to 3 attrs |
