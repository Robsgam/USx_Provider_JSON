...
| NY_NYSPIN_EJUSTICE | providers/NY_NYSPIN_EJUSTICE/ | v4.1 | 81P/0F/0W/0LIM -- v4.1 (2026-07-07) metadata field-completeness: DGRP (DL Name Search) broken out to its own DL NAME SEARCH card with DGRP-suffixed Name/DOB/Sex + all 7 previously-missing optional fields (AddressStreet/City/State/Zip, Age, MessageContinueKeyCode, MiscellaneousDescriptiveText), autoSelect=TRUE (own field pool removes the v2.8 shared-Name co-fire cause -- LIVE-VERIFY isolation); removed over-inclusions Gun ImageIndicator + Boat RelatedHitSearchIndicator (not in those queries' metadata). Person/Firearm/Boat re-test from T1; Vehicle/Article v4.0 captures carry over. Full history: `providers/NY_NYSPIN_EJUSTICE/docs/CHANGELOG_NY_NYSPIN_EJUSTICE.md`. | PascalCase, 9 cards (Veh 2, Per 4 [Options+DL+DL-Name-Search+DH], Gun 1, Art 1, Boat 2), 17 combos, 7 QIDMs, DGRP own card+full metadata field set (v4.1, autoSelect), DH OOS combos (DALHOUT/DALLOUT) with State EXISTS/NOT_EXISTS routing, Vehicle plate OOS combo (RVEHOUT), NyNyspinTransactionName visible on DH (default DALL), PurposeCode default C on DH OOS combos, Choice-set OOS pattern (LIMIT #36), DH-suffix+one-directional queriesToDeselect, CAD defaults, State no-default (LIMIT #30), identifier-priority guardrails (Plate>VIN, OLN>Name DL+DH, Hull>Reg) |
