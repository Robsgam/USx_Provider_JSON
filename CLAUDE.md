...
## Provider Status (updated 2026-06-29)
...
| CA_CLETS | providers/CA_CLETS/ | v2.11 | 75P/0F/0W/0LIM -- v2.11 (2026-06-29): IN.L1 (name in-state) + ID.L1 (OLN in-state) removed from DriverLicenseQuery -- CommSys auto-dispatches pure (In) queries, consistent with Vehicle pattern (Vehicle (In) combos 1-3 also not built); RegistrationState EXISTS condition added to all 6 NLTS combos (NLTS.DQ/NLTS.DQ.N/NLTS.RQ.V/NLTS.BQ.N/NLTS.BQ.H/NLTS.BQ.R) -- platform fires on primaryFieldReference presence, not all-set[] presence; without this NLTS combos shadowed in-state equivalents; verify_build CHECK 16 (combo reachability/shadow detection) added to pipeline. v2.10 (2026-06-26): VehicleMakeName result-mapping code source corrected VEHICLE/VehicleType -> attributeType=VEHICLE_MAKE/codeTypeSource=NCIC (RND-62365). | purposeCode (CAD-aligned fieldId), DH-suffix fieldIds, cross-entity Name on Veh/Gun/Boat, no ImageIndicator, 6 basic queries, yyyyMMdd dates, CAD defaults on IA.QV. DL: 6 combos (NLTS.DQ.N/DQ + IR.QVC.N/O/C/S); pure (In) IN.L1/ID.L1 auto-dispatched by CommSys. RegistrationState EXISTS guards all NLTS combos. IR.QVC OLN cascade: OLN+State->NLTS.DQ, OLN+CII->IR.QVC.O, CII->IR.QVC.C, SSN->IR.QVC.S, Name+Sex->IR.QVC.N. |
