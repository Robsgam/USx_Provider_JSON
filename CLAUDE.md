...
| HI_HCJDC_OFML | providers/HI_HCJDC_OFML/ | v1.9 | 67P/0F/0W/0LIM -- v1.9 consolidated 2026-06-17 (single JSON; BASE/MC split retired); NOT yet imported, test pkg reset to T1 | 6 basic queries (Article/Boat/DH/DL/Gun/VehReg), 17 combos all reachable; single JSON; Person 3 cards (Search Options[State] + Driver License + Driver History, DH-suffix, one-directional deselect); Vehicle/Boat/Firearm/Article 1 card; Type Code dropdown (VEHICLE_TYPE/HI_NIBRS) per HIDLE ref; ImageIndicator=N combo defaults on all 6 VehReg combos (real CAD fix -- not card count); State in DL/DH any[] for OOS; label/title hints (State out-of-state-only, Type Code Hawaii-in-state) |
