...
| NY_NYSPIN_EJUSTICE | providers/NY_NYSPIN_EJUSTICE/ | v4.2 | 81P/0F/0W/0LIM -- v4.2 (2026-07-07) DGRP field refinement: Age relabeled "DOB Search Range (+/- years)" (devdoc: years-from-birthdate fuzzy window, not a person's age) + placed beside DOB; removed MessageContinueKeyCode (DMV-generated paging token, not an initial-search input). AddressStateCode + MiscellaneousDescriptiveText kept (user call). Built on v4.1 (DGRP broken out to its own DL NAME SEARCH card, DGRP-suffixed Name/DOB/Sex + address block, autoSelect=TRUE -- own field pool removes the v2.8 shared-Name co-fire cause, LIVE-VERIFY isolation; removed over-inclusions Gun ImageIndicator + Boat RelatedHitSearchIndicator). Full re-test from T1 on re-import. Full history: `providers/NY_NYSPIN_EJUSTICE/docs/CHANGELOG_NY_NYSPIN_EJUSTICE.md`. | PascalCase, 9 cards (Veh 2, Per 4 [Options+DL+DL-Name-Search+DH], Gun 1, Art 1, Boat 2), 17 combos, 7 QIDMs, DGRP own card+near-full metadata field set (autoSelect; ContinueKey dropped), DH OOS combos (DALHOUT/DALLOUT) with State EXISTS/NOT_EXISTS routing, Vehicle plate OOS combo (RVEHOUT), NyNyspinTransactionName visible on DH (default DALL), PurposeCode default C on DH OOS combos, Choice-set OOS pattern (LIMIT #36), DH-suffix+one-directional queriesToDeselect, CAD defaults, State no-default (LIMIT #30), identifier-priority guardrails (Plate>VIN, OLN>Name DL+DH, Hull>Reg) |
