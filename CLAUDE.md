...
## Provider Status (updated 2026-06-16)
...
| TX_TLETS | providers/TX_TLETS/ | v3.6 | 81P/0F/0W/1LIM -- v3.6 reverts DH ImageIndicator to Y (design intent: manual EmailAddress entry until email-injection handler; ReasonCode=C satisfies second devdoc constraint); v3.5 never imported; re-import v3.6; T1+ testing restarted | 6 cards (Veh 1, Per 3 DH+DL+Options, Gun 1, Art 1, Boat 1), 22 combos, 6 QIDMs, v3.4 poisoned-array fix (merged Img/catchall pairs DL 7→4, DH 4→2; image/email/reason in any[], email manual-entry pending future handler), email→shared OPTIONS card, CPL Name combo, DH-suffix+one-directional queriesToDeselect, TX-specific (DPSI/REG/VIN+FRT), CAD defaults, -SkipRace on RMS |
