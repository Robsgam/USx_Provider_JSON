...
| FL_FCIC | providers/FL_FCIC/ | v7.6 | 92P/0F/0W/0LIM -- 30/30 combos, 6 QIDMs. v7.6 (2026-07-16) Article card cosmetic pass (DEX-1281) + Boat direct-feedback restructuring (no ticket): Article title -> "ARTICLE QUERY by Serial Number, "OR" Owner Applied Number, "OR" NCIC Number, "OR" PCN"; split old 4-field row into SN+OAP row and Type+Image row; dropped "(required)" from Article Type; ImageIndicator -> "NCIC Image - if available". Boat collapsed from 2 cards to 1 (DestState/Stolen/Image moved to a new bottom row); owner Name reordered First-before-Last (matching Person v7.5); ImageIndicator -> "NCIC Image - if available". Both label/layout-only. Article + Boat re-opened for live re-test (first time touched this session); Person/Firearm/Vehicle carry forward open/PENDING (still awaiting their first live capture -- never block_entity-locked yet this session). Prior -- v7.5 direct feedback (Person DL/DH name-order consistency, Vehicle 1-card collapse); v7.1-v7.4: Boat Hull>Reg guardrail completion (v7.1, DEX-971 zero-error pass) + DEX-1278/1279/1280 Person/Firearm/Vehicle cosmetic pass (verify_build CHECK 15 Rule 2 DH-label gate FAIL->Info portfolio-wide + Rule 3 Purpose Code exemption; FRQTitleLienInformation combo intentionally removed, approved-skip, 31->30 combos). Full history: `providers/FL_FCIC/docs/tracking/CHANGELOG_FL_FCIC.md`. | 1-card Vehicle + 1-card Boat (v7.6, both collapsed from 2), Person(DL+DH OOS-only). Devdoc combo order + EXISTENCE-ONLY routing conditions (State NOT_EXISTS / OLN NOT_EXISTS / RelatedHit NOT_EXISTS) for first-match + pool isolation; ALL value-comparison conditions removed (poisoned-array rule, QIDM_REFERENCE Sec 2a); DH KQ out-of-state only; DH+Boat destination state = NCIC dropdown; Boat QB stolen routing via relatedHitSearchIndicator in set[]; identifier-priority guardrails complete (Plate>VIN, OLN>Name, Hull>Reg); Attention auto-populated (handler); RMS Vehicle stripped to 3 attrs |
