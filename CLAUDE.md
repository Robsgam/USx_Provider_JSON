...
| HI_HCJDC_OFML | providers/HI_HCJDC_OFML/ | v4.8 | 65P/0F/0W/0LIM -- v4.8 (2026-07-17, direct feedback, finishes the FL_FCIC/NY_NYSPIN_EJUSTICE-style labeling pass) on top of the same-version 2026-07-15 QW-removal rebuild: dropped the "(DH)" qualifier from every CARD_PER_DH field label (matches DL's phrasing minus the tag, same as FL/NY); Vehicle's Image label -> "NCIC Image - if available"; Boat's State field -> "State (leave blank for Hawaii)" (was "(optional)", a half-applied fix from the 07-15 pass); Firearm/Article/Boat's Search Hit field reworded to name the actual check ("(Y) for NCIC stolen-<entity> check"); Firearm's Make/Caliber/Model get "(with Serial Number, optional)". Person collapsed 3 cards -> 2 (Search Options card eliminated): DH now has its own dedicated RegistrationStateDH field (DriverHistoryQuery's State attribute repointed to it) instead of sharing DL's RegistrationState -- matches every other DH field already being DH-suffixed, and matches FL/NY's fully self-contained DL/DH cards; confirmed via full metadata read that neither query has EXISTS/NOT_EXISTS routing on State, so this is label/layout+field-rename only, no routing change. Label/layout-only elsewhere, no combo/QIDM logic change. All 5 entities remain open pending USx Tenant Testing (rides the already-pending v4.8 re-test from the 07-15 QW-removal rebuild, not a separate cycle). Full history: `providers/HI_HCJDC_OFML/docs/tracking/CHANGELOG_HI_HCJDC_OFML.md`. | 6 basic queries (Article/Boat/DH/DL/Gun/VehReg), 16 combos all reachable; single JSON; Person 2 cards (Driver License + Driver History, each self-contained w/ own State, DH-suffix, one-directional deselect); Vehicle 3 cards (Search Options/Plate/VIN, OOS-first routing); Boat/Firearm/Article 1 card; Type Code dropdown (VEHICLE_TYPE/HI_NIBRS); ImageIndicator=N combo defaults on all VehReg combos; State in DL/DH any[] + VehReg any[] for OOS; identifier-priority guardrails complete (Plate>VIN, OLN>Name DL+DH, Hull>Reg); Name Last-first (v4.0) |
