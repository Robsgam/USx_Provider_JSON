...
```
providers/{PROVIDER}/     -- 21 providers (all galvanized to single-JSON, PascalCase; TX_TLETS_CCH is a variant of TX_TLETS)
...
| SC_SLED | providers/SC_SLED/ | v1.0 | 80P/0F/0W/1LIM | NEVER 0/5 | **FIRST GROUND-UP PROVIDER SINCE THE TOOLCHAIN CHANGED** (2026-09-14) and the first build authored with the `Build-Qidm`/`Build-QidmAttribute`/`Build-QidmCombo` helpers rather than hand-written literals. 9 QIDMs / 18 combos / 6 QIFs. **Three SC keys are COMPOUND and their MessageKey descriptions say so** — `QVRQ` "SC Vehicle Stolen/Reg", `QWDQ` "SC Driver Wanted/Reg", `QBBQ` "SC Boat Stolen/Reg" — against plain NCIC `QV`/`QWA`/`DQ`/`QA`/`QG`. The devdoc declares the mining outright: *"NCIC (QA, QB, QG, QV, QW) and DMV (Person and Vehicle) Tags returned from Data mining"*. ⚠️ **SC_SLED is the ONLY provider in the portfolio that builds a standalone `VehicleStolenQuery`** (FL/HI/NJ each removed theirs; MD never had one) — Rob's call 2026-09-14: *"build both and we can sort it out."* **MEASURED CO-FIRE, not shadowing:** `QV.P` set`[Plate]` equals `QVRQ.P` once PlateType=`PC`/PlateYear are form-prefilled, so one plate fires BOTH QIDMs and the stolen check goes out twice (the AZ `DQPN`/`DQP` exact-collision shape, which ordering cannot separate). Same on Person: `QWDQ` co-fires with `QWA.N`. ⚠️ **Masking stolen is NOT available yet and both mechanisms are disqualified** — `queriesToDeselect` alone is refuted for this shape (NY v2.8: the lower-threshold query sent twice, the higher zero), and `autoSelect=$false` has ZERO tenant-proven carriers (only TX_TLETS_CCH's 8, on a PARKED never-tested provider; its one tenant-observed outcome is the CA_eSUN failure where Send stayed DISABLED). **The first import is the discriminating test.** Person 3 cards (DL / DriverRegistration DR-suffixed / Wanted), cards kept separate by directive. `DriverRegistrationQuery`'s OLN branch is an authority-level DUPLICATE of DriverLicenseQuery's `DQ` (metadata keys both `DQ`, identical `<Requirements>`); its only unique contribution is the DQ **name** path. `DQ` is shadowed by `QWDQ` on a full name fill and reachable ONLY on an OLN-without-name fill. An **OLN-only search gets no wanted check** — SC's design, not a gap (`QWDQ` needs the full name triple; `QWA` defines no OLN-mandatory branch). `AdministrativeMessage` built as its own 6th card, **STATUS: HYPOTHESIS** — the first import must confirm the 5 real entities still render. Boat is a `<Choice>` nested under `<Set>` (hull\|reg) = ONE declared combination, TWO alternatives → synthetic `QBBQ.H`/`QBBQ.R` | [changelog](providers/SC_SLED/docs/tracking/CHANGELOG_SC_SLED.md) |
...
| DriverRegistrationQuery | Driver Registration |
...
- `Build-Qidm -ProviderName <P> -Query <Q> -TargetEntity <E> -QueryLabel <L> -Attributes <arr> -Combinations <arr> -Description <s> [-AutoSelect] [-QueriesToDeselect] [-HandlerFunction]` — **THE QIDM ENVELOPE.** Added 2026-09-14 authoring SC_SLED. ⚠️ **`Build-QidmAttribute` and `Build-QidmCombo` had been here for months and NO provider build script used either** — all 20 hand-write the `PSCustomObject` literals, so the helpers that make a whole defect class unrepresentable were dead code. The missing piece was this outer envelope: with nothing to hang the members off, hand-writing the envelope meant hand-writing the members too. Derives `name` as `<PROVIDER>_<Query>` rather than taking it (a passed name can disagree with `query`, and `audit_sqvr_integrity` plus the log-attribution gates key off it). **REFUSES the shape that once got a whole file rejected:** `conditions` that is not an ARRAY (the CA_eSUN v3.0/v3.1 `Cannot deserialize ArrayList<Combination$Condition> from Object value` import reject, which shipped with ~40 gates green) and a combination carrying `keyRef` instead of `keyReference` (silent null, then rejection). `validate.ps1` type-checks that subtree too, but a gate catches it AFTER the fact — these make it unrepresentable, and impossible beats detected. **PROVEN**: reproduces the SHIPPED `OR_LEDS_VehicleRegistrationQuery` with an identical canonical hash, and `tools\_probes\probe_qidm_helpers.ps1` runs 6 LAW-2 cases (6/6, including a valid CONTROL that must be allowed). Attributes/Combinations accept helper output OR hand-written objects, so a script can adopt it incrementally.
...
## Tools (123 scripts + 21 shared modules in `tools/`, + `tools/_probes/` (11 graduated ad-hoc probes, incl. `probe_qidm_helpers.ps1` -- LAW 2 for the QIDM build helpers, incl. `reconcile_ledger_vs_reality.ps1` -- the records-vs-deployed table, asked for twice -- and `audit_auth_uniformity.ps1`, the RND-71625 AUTHENTICATION-config comparison against CA_CLETS as reference, with an in-memory self-test that refuses to report UNIFORM unless a planted difference is provably detected) + `tools/config/` (9 JSON reference tables, incl. `tenant_scope.json` -- WHICH TENANTS ARE IN SCOPE, exclusion-with-a-reason, ships EMPTY and its `_candidates` block is a proposal only -- and `tenant_map.json` -- the canonical subdomain <-> deptId <-> ledger-name join) + 3 archived tools in `tools/_archive/`)
...
Shared modules (dot-sourced, `_`-prefixed): `_build_rms_bundle.ps1`, `_build_layout_helpers.ps1`, `_build_provider_helpers.ps1`, `_json_canonical.ps1`, `_resolve_provider_json.ps1` (active-JSON resolver `Get-ProviderRootJson` — bare → versioned → `_MC` → `_BASE`), `_resolve_provider_xml.ps1` (metadata-XML resolver `Get-ProviderMetadataXml` — exact `<PROVIDER>.xml` → base provider's XML for a variant → the only XML present → `$null` + warning; **refuses to guess between multiple candidates**, because an alphabetical glob made a gate read CA_CONTRA_COSTA's 6-node JAWS-only excerpt instead of the real 466-node metadata and report green — see `knowledge-base/README.txt`), `_resolve_version_history.ps1` (SHARED git-history walk -- `Get-VersionPaths` / `Resolve-Blob` / `Get-LegacyEntries` / `Get-Index` / `Get-ProvidersInHistory` / `Get-BlobText`. Extracted VERBATIM out of `get_provider_version.ps1` on 2026-09-11 when `audit_tenant_provenance.ps1` needed the identical walk (ENGINEERING_STANDARD 4.4 -- never re-implement an existing parser); the refactor was verified by diffing `-List` output for AZ/HI/NJ/OR AND `-All -IncludeLegacy` before and after, all identical. **Carries the two warnings that make it correct:** `--diff-filter=A` UNDER-REPORTS a version SWAP (git records it as a RENAME, so additions-only found 40 of 257 and reported HI as 2 of 18), and the newest commit TOUCHING a path is usually its DELETION where the blob no longer resolves. **Its `return ,@(...)` comma-guard means a caller must NOT wrap the result in `@()` again** -- that yields a NESTED array, and the consumer then looped ONCE with the whole provider list stringified into a single bogus name and printed "0 artifacts" as though the repo were empty).
...
| `audit_tool_portability.ps1` | **TOOL PORTABILITY SWEEP** — *"shared tools need to work everywhere."* Runs the `-Path`-taking gates against **every** provider and reports whether each can **run and reach a verdict** — not whether it passes. Baseline 2026-09-14: **315 cells (15 gates x 21 providers) / 0 unportable** (this said "12 gates" and was stale). ⚠️ **`-Only` FILTERS PROVIDERS, NOT GATES** — and until 2026-09-14 passing it a gate name matched nothing, exercised **zero cells**, and still printed *"Every shared gate reaches a verdict on every provider"* with exit 0. That is the vacuous-pass class this tool exists to catch, occurring inside it (ENGINEERING_STANDARD 4.3: "found nothing" and "never looked" must not print the same line). A 0-cell run now FAILs and exits 1, and the success line carries its own denominator. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Only <provider-list>` `-OutFile` |
...
| `audit_deploy_guards.ps1` | **PROVE THE ONLY WRITE PATH REFUSES** (composed into `doctor.ps1`). LAW 2 applied to `deploy_probe.js` — the one file in this project that can change someone's tenant. Its entire safety claim is *"every guard can refuse"*, so this **executes** the exported `runGuards` against a real DOM with planted faults: no deptId · deptId mismatching the URL · modal/textarea/button absent · textarea readOnly · **the modal's own target field disagreeing** · payload missing, too small, non-JSON, bundle-less, ENTITIES-less, unstamped, wrong provider, wrong version · **LIVE without `liveConfirmed`** · operator abort. ⚠️ **It carries its own CONTROL** — one deliberately VALID request that must be ALLOWED, because a guard set that refused *everything* would score 100% on the faults while blocking every real deploy. ⚠️ **It asserts its own denominator** (`CASES n`; fewer verdicts parsed than declared = FAIL) — earned, because `--dump-dom` glues the first `<pre>` line to the tag and an anchored match silently dropped the control case, reporting 19 lines for 20. ⚠️ **It detects an INERT TEST** by snapshotting inputs before/after mutation — earned, because the `payload unstamped` case targeted `v7.24",` while the JSON holds `v7.24"}`, so the replace was a no-op and the case blamed the guard for something never exercised. Baseline 2026-09-11: **46 declared / 46 parsed / 46 correct / 0 broken** -- the last 6 assert THE BATCH SEAM: the resolver, `targetReady`, `modalIsOpen` and `bundlePreflight` all driven against a REAL IFRAME DOCUMENT, with a control proving the iframe is genuinely not `document`. Nothing runs a batch yet; this exists so the eventual *"update all FL_FCIC tenants"* goal stays a LOOP rather than becoming a rewrite of the only file that can write to a tenant -- raised from 20 after the fixture was found to be INVENTED rather than measured (one anonymous numeric input, exactly the shape the resolver expected), so it scored 20/20 while the real page refused every dry run. The DOM now comes from `usx_admin_dialog_*.json` and 11 cases assert `findTargetField`/`targetReady`/`modalIsOpen` return values DIRECTLY -- driving them through `runGuards` cannot discriminate, because a null target field and a mismatched one both produce a refusal. Proven by reverting each defect: the old heuristic scores **2 BROKEN**, a presence-only `modalIsOpen` scores **1 BROKEN**. See `knowledge-base/TOOL_REFERENCE.txt`. | `-OutFile` `-Quiet` |
...
| `emit_import_job.ps1` | **THE IMPORT JOB FILE -- the reviewed artifact that authorises an import.** Rob 2026-09-11, after driving the first automated import by hand: *"this is still too clunky  i want the import process to be run by a json you create  kinda of like a import job file.  then i runi t via the console  it updates based on what we discused here."* ⚠️ **The clunk was not keystrokes -- it was that the operator ASSEMBLED THE DECISION AT THE KEYBOARD** (open a tenant, read a panel, judge a dry run, press a red button). Decision and execution were one act, so there was nothing to review beforehand and nothing to diff afterwards. Now the decision is a FILE: this writes `providers\IMPORT_JOB.json`, a human reads it, `serve_plans` serves it at `GET /job`, and `__usxJob()` in the tenant console executes ONLY what it says -- still **one tenant per call** (the import goes through that page's own modal), printing the next target's url. **Plans on CONTENT, never labels** -- a target is queued only when a bundle HASH differs from the repo build, because four tenants carry stale descriptions over current content and a label-driven job would queue no-op imports that each archive a test package. ⚠️ **THREE REFUSALS:** a `NOT-OUR-BUILD` config is never queued silently (`-Force` required, and the override is recorded IN the job so it is visible to the reader rather than living in a shell history); an **EMPTY job FAILs rather than being written**, because "nothing to do" and "the tool found nothing" must not look the same; and every target records `expectBundlesNow`, which the browser re-checks against the live page and **refuses the row if the tenant changed since the job was cut** (safeguard 2 -- what stops us overwriting a concurrent change). A LIVE tenant is queued but carries `liveConfirmed:false` and cannot run until that is set BY HAND in the file. Baseline 2026-09-11: `-All` queues **6** (amyblair, hawaii-dle LIVE, hdle-foundation, newarkpd-foundation, onscene, practice-bertanzini), 59 not queued. | `-DeptId <id[,id]>` `-Provider <NAME>` `-All` `-DryRunOnly` `-Force` `-TenantDir` `-OutFile` `-Quiet` |
| `report_import_plan.ps1` | **WHAT WOULD AN IMPORT ACTUALLY CHANGE — read-only, judged by CONTENT.** Rob 2026-09-11: *"we need to have you generate json import plan either mass or single  i will drive that moving forward adn then have it automated at the sinlge click."* Step 4 of the import-automation roadmap, and deliberately **contains no write path** — `Import JSON` is on the extension's DESTRUCTIVE denylist by design. ⚠️ **IT PLANS ON CONTENT, NOT LABELS, AND THAT CHANGES THE ANSWER:** planning off bundle descriptions would have queued **four tenants that need nothing** (`usx-hi-hcjdc-ofml` label v4.19 / content == repo v4.20; `usx-ny-nyspin-ejustice` and `ny-nycapss-foundation` label v4.24 / content == v4.26; `usx-or-leds` label v2.5 / content == v2.6) — each a no-op import that archives a test package and burns a re-test cycle. Nothing enters the queue unless its content differs. ⚠️ **Hashing goes through `_bundle_identity.ps1`** because the platform re-serializes on export (`"conditions":null` where our build omits the property), so a raw byte compare reports a difference on every tenant including current ones — **and any future import-VERIFY step must use the same module or it will report failure on a successful import.** ⚠️ **Three refusals:** it never queues a **NOT-OUR-BUILD** config (overwriting a config we did not author is a different decision needing a human); never queues on a label difference alone; and **0 tenant configs FAILs**, because an empty plan must not read as "nothing to do". Honours `tenant_scope.json` and **prints its denominator** — excluded tenants appear in their own section with the reason, never dropped. Emits the 3-step acceptance test per row, naming which step actually proves the import landed. Baseline 2026-09-11: **CURRENT 26 / QUEUE 6 / not-our-build 32**. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Provider <name>` `-TenantDir <dir>` `-IncludeDeactivated` `-OutFile` `-Quiet` |
| `verify_tenant_import.ps1` | **DID THE IMPORT ACTUALLY LAND? Proof, per bundle, from hashes.** Rob 2026-09-11: *"maybe a skill that imports and runs the export o check what the actual cversion is with proof."* Compares three things per bundle — **BEFORE** (archived pre-import) / **AFTER** (fresh pull) / **REPO** (the build we meant to install). Works identically whether the import was done BY HAND or by a future tool, which is the point: **if it cannot prove a manual import landed, it cannot verify an automated one.** ⚠️ **FOUR REFUSALS, each proven against replica fixtures via `-TenantDir`:** `UNPROVEN` when no BEFORE was archived — and it deliberately refuses to fall back on "AFTER matches REPO, therefore it worked", because a tenant **already** at the target version satisfies that with no import at all; **`THE IMPORT DID NOT LAND`** when every bundle is byte-identical to BEFORE, *whatever the label now says* — the failure a label check misses completely; **`LABEL-ONLY CHANGE`** counted as FAILURE, not success; and `FAIL` when content changed but does not match the repo build (*worse than unchanged, because it looks like success*). Exits **1** on all four so it can be chained. ⚠️ **The verdict is CONTENT, never a version string:** a label check passes on a no-op, and a raw byte compare FAILS on a correct import because the platform re-serializes — both avoided via `_bundle_identity.ps1`. Paired with the **`usx-deploy`** skill and with `ingest_tenant_configs.ps1`, which archives a changed prior config to `_versions\tenant_exports\_before\` so the second export cannot destroy the first. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Tenant <subdomain\|deptId>` `-ExpectedProvider` `-ExpectedVersion` `-TenantDir <replica>` `-OutFile` `-Quiet` |
| `propose_ledger_patch.ps1` | **PROPOSE LEDGER CORRECTIONS. NEVER WRITE THEM.** Rob: *"i will help align them with our ledger as needed"* -- so the tool reports the delta and he stays the author. ⚠️ **The ledger's rows are not data, they are PARAGRAPHS OF ADJUDICATION** (*"SDSO runs v1.0, NOT v3.3"*, *"Lafayette is hand-built by engineering"*, *"TX_TLETS_CCH is PARKED"*); a tool that rewrote them would destroy reasoning it cannot reconstruct, silently. Emits line number + the exact text now there + the evidence + the minimal edit, into `providers\LEDGER_PATCH.md`. **Proposes only what CONTENT proves** -- every comparable bundle must hash equal to a repo build (`_bundle_identity`), because a label is not a version; a merely-LABELLED tenant is listed separately with the reason, since *"we could not prove it"* and *"it agrees"* must never look the same. ⚠️ **THREE CLASSES THAT ARE NOT CONTRADICTIONS, each earned:** a row that states NO version (the first run reported **8 DISAGREEs and every one was the tool's own logic** -- the matches were Section B.0's name-correlation table, whose cells carry a PLATFORM COUNTER by design); a filename with no parseable deptId (a non-matching `-replace` returns its INPUT, so one stray export produced a proposed ledger row reading `(dept CA_eSUN_dept69510509021_20260910-163019)` -- a proposal a human might paste must never contain a value the tool did not parse); and a row naming SEVERAL versions, where **only one is the claim and the rest are history** -- the FL row names v7.24 twice plus v7.17/v7.19/v7.20/v7.23 in prose, and the first cut emitted an EDIT for each, i.e. would have rewritten the history to read as though it had always been current. So a literal EDIT is offered ONLY when the row names exactly one version; otherwise it says CHOOSE and names the count. **Proven able to fire** against a mutated ledger replica via `-LedgerPath` (DISAGREE 1, correct EDIT on a single-version row, CHOOSE on the multi-version one) -- which is how the history-rewrite hazard was found at all, since the clean run reports DISAGREE 0. Baseline 2026-09-12: **26 content-proven / agree 2 / DISAGREE 0 / no ledger row 16 / row states no version 8**. | `-Tenant` `-TenantDir` `-LedgerPath <replica>` `-OutFile` `-Quiet` |
...
| `refresh_tenant_reports.ps1` | **REGENERATE THE DERIVED TENANT REPORTS, AND NAME THE ONES THAT CANNOT BE.** Rob 2026-09-12: *"i am looking at the reports in the privers folder and they are not updated with current info."* He was right and the content was WRONG, not merely old: `IMPORT_PLAN`, `TENANT_PROVENANCE`, `LEDGER_VS_REALITY` and `TENANT_BUNDLE_VERSIONS` all still described `usx-fl-fcic` as carrying a CA_eSUN bundle and NOT-OUR-BUILD, hours after an import had moved it to FL_FCIC v7.24 and the change had been proven by content hash. ⚠️ **The cause is that nothing owned the SET** -- each report is produced by its own tool with its own `-OutFile` at whatever moment someone happened to run it, so a document being read to make a scope or ledger decision is stale exactly when it matters. ⚠️ **TWO CLASSES, AND CONFLATING THEM IS THE FAILURE IT EXISTS TO PREVENT:** *derivable locally* (everything computed from `_versions\tenant_exports\` + the repo) is regenerated unconditionally; *needs a fresh browser pull* (census chunks, versions sweep, department index) **cannot** be refreshed from disk and is REPORTED with its age and the button that would refresh it -- never regenerated from stale input, never left looking current. Uses the newest tenant export as a **DATA CLOCK**: any derived report older than it is describing a state that no longer exists. Verifies each rewrite by **WRITE TIME, never `Test-Path`** (a leftover satisfies `Test-Path`, which is how a stale officer-guide PDF once shipped under a green success line) and FAILs on a generator that did not rewrite, because such a file still LOOKS current. Deliberately does NOT touch `IMPORT_LEDGER.md` (hand-authored adjudications) or `TENANT_INVENTORY.md` (a narrative, not a tool output). `-CheckOnly` reports staleness without changing anything. | `-CheckOnly` `-Quiet` |
| `identify_notours_provider.ps1` | **WHICH USx PROVIDER WOULD HAVE TO SUPPORT EACH NOT-OUR-BUILD CONFIG -- and which tenants are identical, so one fix covers them all.** Rob 2026-09-12: *"i want you to beter idetify the usx provider it mostly aligns with adn say which ones are identical in the saem family ergo same usx provder support and any one off with associated state."* Grouped BY PROVIDER (the unit of support), FAMILIES within it (byte-identical provider bundle = one config on N tenants, so one defect is N defects), one-offs named with their STATE. Baseline 2026-09-12: **LA_LEMS/Louisiana 22 · CA_eSUN/California 5 · HI_HCJDC_OFML/Hawaii 1 · FL_FCIC/Florida 1 · 2 with no recognised routing key**. ⚠️ **IDENTITY IS THE ROUTING KEY, NOT A SIMILARITY SCORE, AND THE FIRST TWO INSTRUMENTS WERE BOTH WRONG.** (1) Scoring keyRef+query overlap by Jaccard reported **23 tenants carrying `LA_LEMS` bundles as aligning with NJ_NJCJIS** -- Louisiana matched to New Jersey while our own LA_LEMS build sat there. Cause: these configs use DESCRIPTIVE keyRefs (`BirthDateNamePurposeCodeSexCodeRegistrationStateAttention`) where ours are short codes, so overlap was ZERO everywhere and the score rode on generic query names, after which Jaccard crowned the build with the SMALLEST signature. **A similarity measure over a dimension where nothing can match does not return "no match" -- it returns noise shaped like an answer.** (2) Switching to the bundle HEADER's `provider` field then filed `fullwooddemo` under New Jersey, because its header says `NJ_NJCJIS` while **all nine configurations inside declare `CA_eSUN`**. Identity now comes from the majority across CONFIGURATIONS (what each QIDM actually declares) and a disagreeing header is **reported as an inconsistency rather than silently resolved**. ⚠️ **No recognised routing key = NO MATCH, never the nearest one** -- `RecordsArchive` is not a CJIS provider, and assigning it would invent a support obligation. Also flags a bundle whose NAME differs from its routing, and a config with **no ENTITIES bundle** (`1836` -- the forms cannot render; a fragment, not a working config). | `-TenantDir` `-OutFile` `-Quiet` |
| `report_notours_reuse.ps1` | **OF THE CONFIGS WE DID NOT BUILD, WHICH ARE COPIES AND WHICH ARE ONE-OFFS?** Rob 2026-09-12: *"for all the json you found that are 'not our builds' can you narrow down if any are being resued and which ones are unique."* ⚠️ **The tenant COUNT does not answer this.** A not-ours config on ONE tenant is a hand-built one-off; the same config on FIFTEEN is a de-facto product, where one defect is fifteen defects. ⚠️ **IT COMPARES ON TWO AXES because one is misleading alone:** *whole config* (byte-identical after canonicalisation) and **provider bundle only** -- the useful one, because two tenants can differ in ENTITIES/RMS while running the IDENTICAL bundle that talks to the state system, and whole-config grouping would report those as unrelated one-offs and understate the reuse. Measured 2026-09-12: **31 not-ours tenants carry just 11 distinct provider bundles** -- one LA_LEMS bundle on **15** tenants (under 4 different whole configs), another on 3, and **5 genuine one-offs**. Hashes go through `_bundle_identity.ps1` (description excluded, platform nulls normalised) because a raw byte compare cannot work -- the platform re-serializes on export. "NOT OURS" is MEASURED: no `Provider configuration for <P> vX.Y` description, which is EVIDENCE (it is how Lafayette was confirmed hand-built), not a gap. ⚠️ **It states what it does NOT know:** identical content is equally consistent with central maintenance and with one file hand-copied to many tenants -- and the second is the support exposure worth knowing about. 0 not-ours configs FAILs rather than reporting an empty analysis as a result. | `-TenantDir` `-OutFile` `-Quiet` |
| `report_tenant_groups.ps1` | **EVERY TENANT WE FOUND, GROUPED FOR A HUMAN TO DECIDE ON -- HTML + PDF.** Rob 2026-09-12: *"i need the list of found tenants so i can group them. start by grouping the provider we created adn the ones we did not   then with the ones we did not, there should be a few that are obvious real tenants and the rest likley are just deleveopment and testing tenants  make a pdf."* Scoping is the blocker (`tenant_scope.json` ships EMPTY, so every report and every import job still carries tenants already decided against) and nobody can decide from a 64-row JSON. ⚠️ **IT SEPARATES MEASURED FROM GUESSED, ON THE PAGE.** *Ours / not-ours is MEASURED* -- the bundle description carries our `Provider configuration for <P> vX.Y` stamp, corroborated by content hash against the current repo build; **absence of that stamp is EVIDENCE, not a gap** (it is how Lafayette was confirmed hand-built by engineering). *The real-vs-development split is a HEURISTIC* -- the only guessed thing in the document -- so every B2 row prints **the signal that placed it** and nothing acts on the split. ⚠️ **A LIVE tenant is never filed as dev on its name** (status outranks the name), and the report explicitly warns that a customer's TRAINING tenant is not a sandbox -- `lafayettela-sherifftraining` is Lafayette Parish, and excluding it would hide a supported environment. ⚠️ **The PDF is verified by WRITE TIME, never `Test-Path`** -- a leftover from a previous run satisfies `Test-Path`, which is exactly how a stale officer guide once shipped under a green success line -- and the wait POLLS rather than sampling once. ⚠️ **Widening the `demo` pattern was found by READING the output, not its counts**: the first cut required a separator, so `erich-demo1` and `fullwooddemo` were filed as REAL TENANTS. Baseline 2026-09-12: **64 tenants / OURS 33 / NOT OURS 31 (real-looking 10, dev-test 21) / 0 deactivated in the pulled set**. | `-TenantDir` `-OutFile` `-PdfFile` `-IncludeDeactivated` `-Quiet` |
...
| Tool | Purpose | Key flags |
|---|---|---|
| `extract_metadata_reference.ps1` | Generates METADATA_REFERENCE.txt from XML + JSON (field definitions, combo requirements, coverage). ⚠️ **`-Path` IS OPTIONAL as of 2026-09-14, and making it so removed a chicken-and-egg block on every GROUND-UP build:** this file carries the combination requirements the Source Authority table below says to read BEFORE writing a build script, yet the built JSON was a MANDATORY parameter -- so a brand-new provider could not obtain its reference until after it had been built. Found on SC_SLED, the first from-scratch provider since the toolchain changed. Without `-Path` the metadata sections are produced IN FULL and the coverage map reports **NOT-YET-BUILT** rather than being faked or silently omitted; the devdoc is located beside the XML instead of beside the JSON. Existing callers pass `-Path` and are unaffected -- **proven byte-identical before/after on NJ_NJCJIS and OR_LEDS**, which is the only reason to believe an edit to a shared tool is inert. | `-XmlPath <xml>` `[-Path <json>]` `-OutFile` `-All` `-DevdocPath` |
...
| `ingest_tenant_roster.ps1` | **WHAT CHANGED ON THE PLATFORM SINCE LAST TIME — new tenants, renames, status changes, from ONE page load.** Rob 2026-09-11: *"the intial scan and inventory will be a one time thing. moving forward we need to figure out how to extract 'new' tenants on a regular basis and be able to audit the tenants on demand."* ⚠️ **DIFFS THE INDEX, NOT THE CENSUS.** The full census visits all 1,785 configuration pages (20–25 min); the department index is a **single** page load and already carries deptId/subdomain/status/analyticsAlias/cadSubdomain. So a recurring audit is: pull the index → diff → census **only what changed**. Seconds, not hours. Baseline lives in `tools\config\tenant_roster.json` (COMMITTED, metadata only). ⚠️ **FOUR REFUSALS, all proven to fire:** a **BOOTSTRAP is not 1,785 discoveries** (no baseline ⇒ everything reads new, so it reports NO findings and says so); a **RENAME is not a new tenant** (same deptId, moved subdomain — the `hawaii-dle` lesson, where nothing in "HDLE LIVE" implies that subdomain); a **short index is a truncated capture, not a mass deletion** (shrink beyond `-MaxShrinkPct 10` suppresses departures entirely — verified real, not inert: the same 1400-of-1785 input reports 385 departures at `-MaxShrinkPct 50`); and a **read-only run does not move the baseline** (`-Update` required, so a diff can be inspected before being absorbed and the same change cannot silently stop being reported). Emits a paste-ready dept-id list of the new **non-deactivated** tenants to census next (Rob: *"we can filter out deavtivated tenants"*). See `knowledge-base/TOOL_REFERENCE.txt`. | `-Path <file>` `-Update` `-MaxShrinkPct <n>` `-OutFile` `-Quiet` |
...
| `ingest_tenant_versions.ps1` | **WHAT VERSION IS ON EVERY TENANT, AND DOES IT MATCH WHAT WE SAY?** Rob's ask in full was *"visit each configuration page to 1 determimn if it has a usx provider installed **and download to compare what version for cross checking**"*; `ingest_tenant_scan.ps1` did the presence half and I then offered to export a HANDFUL -- *"i thought you were supposed to export every json you find."* This is the other half. Reads the extension's `usx_admin_versions_*.json` (button 6 clicks each tenant's own **Export JSON** in a hidden iframe) and does a THREE-WAY compare: **MEASURED** (the tenant's export) vs **LEDGER** (what IMPORT_LEDGER section B claims, via `tools/config/tenant_map.json`) vs **REPO** (current versioned root JSON), naming which pair disagrees. WARNING: a non-`VERSION-READ` verdict is UNRESOLVED, never "no config"; **absence of our `Provider configuration for <P> vX.Y` description is EVIDENCE, not a gap** -- it is how Lafayette was confirmed hand-built, so it reports NOT-OUR-BUILD; the platform counter is never printed as a version; 0 files FAILs. Partials overlap by design (button 6 saves every 3), so rows are keyed by deptId and the LAST-WRITTEN file wins. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Path <dir>` `-OutFile` `-Quiet` |
| `ingest_tenant_configs.ps1` | **THE BASELINE RECORD OF WHAT IS DEPLOYED WHERE — and the BEFORE that makes an import verifiable.** Rob 2026-09-11: *"i want you to scan and pull all the jsons so we have an actual record of what is where"* and *"the idea is for the tool to eventually execute the imports, then run an export, then compare the 2 to confirm the json update takes place."* The first 64-tenant sweep read every config and **discarded all 64**, keeping a version string and a byte count — ~3 hours of page loads spent to keep a number. Reads the extension's per-tenant `usx_tenant_config_*.json` (button **6b**), extracts each payload to the **gitignored** `_versions\tenant_exports\`, and emits a METADATA-ONLY inventory (`providers\TENANT_CONFIG_INVENTORY.json`). ⚠️ **Computes a CANONICAL SHA-256, because a byte compare CANNOT verify an import** — the platform re-serializes, so a 246KB export and a 928KB repo JSON at the SAME version are expected to differ; only equality would be surprising. ⚠️ **THREE classes, not the two Rob asked for:** `OURS` / `PROVIDER-NOT-OURS` (bundle present, no version string — Lafayette, `sdso`, `gordo`, 24 byte-identical LA_LEMS demo copies) / `UNKNOWN`. Folding the middle 32 into OURS claims ownership of configs we did not author; folding them into NON-USx hides the support exposure. ⚠️ **Refuses to write a payload to a tracked path** (consults `git check-ignore`; one tenant export reached pushed history once and needed a force-push). **Does NOT touch `IMPORT_LEDGER.md`** — hand-authored, Rob aligns it: *"i will help align them with our ledger as needed."* Reports mixed-version tenants (>1 version string for one provider) and identical-hash groups. 0 files FAILs. Three real bugs were caught by its own fixture before first use: a duplicate `version` key in the extension literal (JS keeps the last, silently), a 5.1-fatal backtick+`$(if)` inside `-f`, and `Group-Object` on ordered hashtables producing a FALSE "4 tenants share hash" with an empty hash. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Path <dir>` `-OutFile` `-Quiet` |
| `audit_tenant.ps1` | **ONE TENANT, EVERYTHING WE KNOW, ON DEMAND.** Rob 2026-09-11: *"be able to audit the tenants on demand."* Takes a **subdomain OR a deptId** (a human knows one, the data is keyed on the other) and assembles identity + status from `tenant_roster.json`, the ledger claim from `tenant_map.json`, the deployed config from `_versions\tenant_exports\`, **content provenance** from `bundle_hash_index.json`, and the repo's current version — five files that individually may as well not exist when someone asks "what is on Newark?". ⚠️ **FOUR REFUSALS:** *"NEVER PULLED" is not "no provider"* (it prints the exact deptId to paste and the command to run); *a LABEL is never reported as a version* — label and content are shown side by side **per bundle** and never collapsed, because that single "tenant version" field is what made `usx-hi-hcjdc-ofml` read BEHIND when it is CURRENT and `practice-bertanzini` read CLEAN when its provider bundle is older than the ledger claims; *no version string = NOT OUR BUILD* (positive evidence); and *an unresolved tenant FAILs* rather than printing an empty dossier as a healthy one. MANUAL-ONLY (`tools\config\manual_only_gates.json`) — no orchestrator knows which tenant is being asked about, and it correctly exits 0 on a never-pulled tenant, which would be vacuous as a gate. See `knowledge-base/TOOL_REFERENCE.txt`. | `-Tenant <subdomain\|deptId>` `-OutFile` `-Quiet` |
| `audit_tenant_provenance.ps1` | **WHICH BUILD IS THIS BUNDLE, ACTUALLY — by CONTENT, against every version in git.** Rob 2026-09-11: *"we have all the curren tjsons in our repo so figuring out what the combination of versions shouldn't be hard"* → *"do that  that should be part of the ability to generate any version of the json like we worked on earlier."* Hashes every bundle of every historical build (via the shared `_resolve_version_history.ps1`, reading blobs with `git cat-file` so ~540MB is never extracted) into a **blob-keyed cache** (`_versions\bundle_hash_index.json`, gitignored, 560KB), then identifies each tenant bundle against it. **259 artifacts / 21 providers / 385 distinct bundle contents; first build ~10 min, cached re-runs instant.** ⚠️ **IT REPLACES A LABEL-BASED READ THAT WAS WRONG IN BOTH DIRECTIONS.** Our version rides in the DESCRIPTION of up to three bundles and **imports are PER-BUNDLE**, so `usx-az-azdps` RMS labelled `AZ_AZDPS v3.4` (content byte-identical to v3.12 → reported a mixed install, nothing was wrong), `usx-hi-hcjdc-ofml` ENTITIES v4.19 / PROVIDER v4.20 (→ reported BEHIND, it is CURRENT), and `practice-bertanzini` ENTITIES v4.9 / PROVIDER v4.8 (→ reported CLEAN because the stale label matched the ledger — the dangerous direction). ⚠️ **THE HASH MUST EXCLUDE THE DESCRIPTION AND NORMALIZE PLATFORM-ADDED NULLS.** The platform emits `"conditions":null,"defaults":null` where our build omits the property (+324 bytes on one AZ bundle) — before that normalization BOTH current provider bundles on ALL-PASS tenants read "matches no known build", which is what exposed it. **This is also why an import can never be verified by a byte compare.** ⚠️ **Three refusals, each earned on its first run:** an EMPTY index FAILs rather than reporting all 64 tenants unmatched; `PROVIDER NOT INDEXED` is distinguished from `MATCHES NO KNOWN BUILD` (`usx-la-lems` read unmatched only because `LA_LETTS_OFML` exists solely in the pre-versioned era — it resolves under `-Providers LA_LETTS_OFML -IncludeLegacy`); and a malformed provider list FAILs loudly. **A bundle matching MANY builds is the normal case and is itself the answer** — the RMS bundle is identical across versions, so a stale RMS label is HARMLESS. Baseline 2026-09-11: **0 bundles whose label names a build their content is not.** See `knowledge-base/TOOL_REFERENCE.txt`. | `-Providers <list>` `-TenantDir <dir>` `-Rebuild` `-IncludeLegacy` `-OutFile` `-Quiet` |
...
| `serve_plans.ps1` | **THE LOCALHOST BRIDGE** the extension reads the repo through, so nothing the browser uses is hand-picked. `GET /plan/<P>` + `/scope/<P>` (test driver), `GET /build/<P>` (the deploy payload, repo artifact byte-for-byte -- `verify_tenant_import` compares the tenant against the REPO build, so a payload from anywhere else would have it comparing a tenant to a build it was never given), and **`GET /target/<deptId>` -- WHICH PROVIDER THAT TENANT IS SUPPOSED TO RUN**, added 2026-09-11 after Rob: *"typing fcic in tath window is not right you should already know what the tenatn is supposed to be."* He is right on SAFETY, not convenience: a typo in a provider box imports the wrong provider and **every deploy guard still passes** (valid stamped build, matching deptId, matching modal field) because nothing compared the payload's provider to the tenant's intent. Authority order is stated in every answer -- `explicit-map` (`intendedProvider` on the `tenant_map.json` row) -> `usx-subdomain` (the slug encodes it) -> **409 REFUSE**; an unrecorded tenant is not a deploy target. ⚠️ **The INSTALLED bundle is never the authority**, only context: `usx-fl-fcic` was carrying a CA_eSUN bundle, so "what is installed" names exactly the wrong provider on the one tenant we deployed to first. Casing is canonicalised off disk (derivation yields `CA_ESUN`; the bundles say `CA_eSUN`, and a case-insensitive filesystem hides that until something string-compares). Provider resolution is exact -> UNIQUE prefix -> refuse, never guess. Re-reads its config per request on purpose -- a server started yesterday served pre-change code for a day. ⚠️ **VERIFY A RUNNING INSTANCE BY CURLING ITS ENDPOINTS, NOT BY ITS START TIME** — on 2026-09-14 a 3-day-old process looked stale by timestamp (started 2 minutes before the script's own last commit, the textbook shape) and answered every endpoint correctly. **`GET /roster` added 2026-09-14** — serves `tools/config/tenant_roster.json`, the committed tenant BASELINE, purely so the panel's **button 7b** can DIFF the department index instead of re-censusing 1,785 configuration pages. Rob: *"fix up the extension so you can easliy rescan everything and update you tenenat list."* The index is ONE page load carrying deptId/subdomain/status, so the recurring audit is read-index → diff → census only what moved (seconds vs 20-25 min) — which `ingest_tenant_roster.ps1` has prescribed since it was written; the browser simply had no way to READ the baseline. **404s on a missing baseline rather than returning empty**, because a bootstrap must not read as 1,785 discoveries. Metadata only — the roster carries no config payloads, which is why it is committed. | `-Port <n>` |
| `install_git_hooks.ps1` | **MAKES A GATE UNBYPASSABLE BY HABIT.** Copies `hooks\` into `.git\hooks\` (which is NOT version-controlled, so a hook written there exists on exactly one machine and silently does not exist anywhere else). Currently one hook: **pre-commit**, which runs `audit_extension_syntax.ps1` whenever `automation/extension/*.js` is staged and BLOCKS the commit on a parse failure. ⚠️ **The gate was never the weak point -- the habit was.** On 2026-09-04 an apostrophe inside a single-quoted `console.log` in `usx_lib.js` closed the string early and the driver + capture tools were DEAD FOR FIVE DAYS; `audit_extension_syntax` was built to catch exactly that. On 2026-09-11 the same mistake was made in `deploy_probe.js`, the gate DID report `[FAIL] deploy_probe.js -- missing ) after argument list`, **and the commit went through anyway** because it had been chained behind a `grep` that matched the output whether the verdict was PASS or FAIL. Reading a verdict beside a commit is not branching on it. `--no-verify` still works on purpose: a hook nobody can override in an emergency gets uninstalled instead of respected. `-Verify` reports STALE by CONTENT HASH, never `Test-Path` (a leftover satisfies that), and an empty `hooks\` FAILs rather than reporting a vacuous success. Proven able to block: appending an unterminated string to `deploy_probe.js` and committing is refused, and no commit object is created. | `-Verify` |
| `watch_imports.ps1` | **WAIT FOR THE AFTER-EXPORT AND PROVE THE IMPORT, WITHOUT BEING ASKED** -- step 1 of closing Rob's loop: *"execute the imports, then run an export, then compare the 2 to confirm the json update takes place."* The import became one click; the PROOF was still three manual steps, and the middle one is the step that gets skipped. That matters more than convenience: `verify_tenant_import` reports **UNPROVEN** without a fresh AFTER and deliberately refuses to fall back on "AFTER matches REPO, therefore it worked" -- a tenant ALREADY at the target satisfies that with no import at all. The panel now auto-exports after a `CLICKED` verdict (button 6b's own GET-only path, scoped to one deptId); this picks that file up and finishes the chain: `ingest_tenant_configs` -> `verify_tenant_import` -> PASS / DID-NOT-LAND / FAIL. ⚠️ **It considers ONLY files newer than its own start** -- the OPPOSITE of `watch_captures`' startup catch-up rule, and for the opposite reason: a capture is evidence whenever it was made, but an AFTER-export is evidence only RELATIVE TO A SPECIFIC IMPORT, so a tenant config sitting in Downloads from an earlier pull must never satisfy it. ⚠️ **A timeout reports TIMEOUT and exits 2** -- "nothing arrived" must not look like "nothing was wrong". ⚠️ **`-Once` is the supported mode**, because a persistent watcher never notifies its supervisor and gets killed by the environment (the lesson `watch_captures.ps1` already carries). Proven able to FAIL end to end: dropping a re-pull of an unchanged config makes it report `THE IMPORT DID NOT LAND` and exit 1, which is the correct verdict for an export with no import behind it. | `-Once` `-DeptId <id[,id]>` `-TimeoutSec` `-PollSec` `-Quiet` |
...
| `usx-deploy` | **Importing a JSON into a tenant and PROVING it landed.** Rob 2026-09-11: *"maybe a skill that imports and runs the export o check what the actual cversion is with proof."* Holds the import→export→compare loop and, more importantly, the **sequencing**: do the first import BY HAND and let the tooling prove it, because *if the verifier cannot prove a manual import landed it certainly cannot verify an automated one* — automating the write first means debugging two unproven halves at once. Holds the step that gets skipped and invalidates everything (**capture the BEFORE**, or `verify_tenant_import` correctly reports UNPROVEN), the two traps that make the naive checks wrong (**a label check passes on a no-op; a raw byte compare fails on a correct import** because the platform re-serializes), the 11 safeguards a future deployment tool must carry, and why `usx-fl-fcic` is the right first subject (currently carrying a foreign CA_eSUN config with no RMS bundle — nothing tuned to clobber, and the import is also a fix). ⚠️ **There is deliberately NO write path in the repo:** `Import JSON` is on the extension's DESTRUCTIVE denylist and `admin_probe.js` is GET-only by construction. Also records the UNRESOLVED question that gates all automation — whether the import payload goes in via a native file dialog (undriveable from page JS), a hidden file input, or a textarea. |
