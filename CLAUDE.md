...
## Provider Status (updated 2026-07-07)

| Provider | Path | Version | Status | Notable patterns |
|---|---|---|---|---|
| NJ_NJCJIS | providers/NJ_NJCJIS/ | v4.8 | 61P/0F/0W/0LIM -- v4.8 (2026-07-01) metadata-driven keyRef rename (DQ/DQN->FULL/FULLN, RQ/RQN->RANDFULL/RANDFULLN per devdoc). Re-import + full re-test from T1 pending. Full history: `providers/NJ_NJCJIS/docs/tracking/CHANGELOG_NJ_NJCJIS.md`. | VehicleStolenQuery NOT built (USER-APPROVED skip; state auto-runs QV, response data-mined via QRDM); VehReg 2 combos (RANDFULL/RANDFULLN), poisoned-array RandomRequest=Y conditions removed (RandomRequest user-controlled in any[]); DriverLicense 2 combos (FULL/FULLN); PascalCase USx fieldIds (CAD/OnScene), Mark43/RMS keys stay camelCase; CAD combo defaults; NCIC state; shared RMS module, RMS Vehicle stripped to 3 attrs |
| HI_HCJDC_OFML | providers/HI_HCJDC_OFML/ | v4.7 | 66P/0F/0W/0LIM -- v4.7 (2026-07-02) NJ/CA-parity modernization (retired phases/, migrated docs/ to 4-category structure, tests/->logs/<Entity>/) + Vehicle OOS-gate symmetry fix (LicensePlateTypeCode EXISTS on RQ, RegistrationState EXISTS on RQV, mirrors M55L/M55S NOT_EXISTS gates). Vehicle re-test required; Person/Firearm/Article/Boat fingerprints unchanged, prior blocks carry over. Full history: `providers/HI_HCJDC_OFML/docs/tracking/CHANGELOG_HI_HCJDC_OFML.md`. | 6 basic queries (Article/Boat/DH/DL/Gun/VehReg), 17 combos all reachable; single JSON; Person 3 cards (Search Options[State] + Driver License + Driver History, DH-suffix, one-directional deselect); Vehicle/Boat/Firearm/Article 1 card; Type Code dropdown (VEHICLE_TYPE/HI_NIBRS); ImageIndicator=N combo defaults on all VehReg combos; State in DL/DH any[] + VehReg any[] for OOS; identifier-priority guardrails complete (Plate>VIN, OLN>Name DL+DH, Hull>Reg); Name Last-first (v4.0) |
| NY_NYSPIN_EJUSTICE | providers/NY_NYSPIN_EJUSTICE/ | v4.5 | 81P/0F/0W/0LIM -- v4.5 (2026-07-07) Person layout tighten: dumped the SEARCH OPTIONS card, folded State/Image into each query card's identifier row (no added height), packed name rows + right-sized columns, reordered per query flow (Person 4 cards->3). DH now self-contained with its OWN DH-suffixed RegistrationStateDH + ImageIndicatorDH (DH OOS routing conditions moved off the shared RegistrationState -> RegistrationStateDH); DL keeps its own RegistrationState/ImageIndicator. All metadata fields retained. v4.4 moved DGRP card to bottom; v4.3 restored MessageContinueKeyCode (metadata is field-authority, not devdoc); DGRP own card, full 10-field set, autoSelect=TRUE (LIVE-VERIFY isolation). v4.1 removed Gun ImageIndicator + Boat RelatedHitSearchIndicator (NOT in metadata). Full re-test from T1 on re-import. Full history: `providers/NY_NYSPIN_EJUSTICE/docs/CHANGELOG_NY_NYSPIN_EJUSTICE.md`. | PascalCase, 8 cards (Veh 2, Per 3 [DL+DH+DL-Name-Search], Gun 1, Art 1, Boat 2), 17 combos, 7 QIDMs, DGRP own card (last)+full 10-field metadata set (autoSelect), DH self-contained w/ own StateDH/ImageDH + OOS combos (DALHOUT/DALLOUT) RegistrationStateDH EXISTS/NOT_EXISTS routing, Vehicle plate OOS combo (RVEHOUT), NyNyspinTransactionName visible on DH (default DALL), PurposeCode default C on DH OOS combos, Choice-set OOS pattern (LIMIT #36), DH-suffix+one-directional queriesToDeselect, CAD defaults, State no-default (LIMIT #30), identifier-priority guardrails (Plate>VIN, OLN>Name DL+DH, Hull>Reg) |
