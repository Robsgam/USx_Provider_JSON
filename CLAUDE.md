...
| HI_HCJDC_OFML | providers/HI_HCJDC_OFML/ | v1.8 | 67P/0F/0W/0LIM -- v1.8 consolidated 2026-06-17 (single JSON; BASE/MC split retired); NOT yet imported, test pkg reset to T1 | 6 basic queries (Article/Boat/DH/DL/Gun/VehReg), 17 combos all reachable; single JSON, Person 2 cards (DL+DH distinct), Vehicle/Boat 1 card each; ImageIndicator=N combo defaults on all 6 VehReg combos (real CAD fix -- not card count); VehicleTypeCode in-state vs PlateType OOS routing; NCIC state |
