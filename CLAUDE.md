...
## Provider Status (updated 2026-06-17)

| Provider | Path | Version | Status | Notable patterns |
|---|---|---|---|---|
| NJ_NJCJIS | providers/NJ_NJCJIS/ | v4.0 | 61P/0F/0W/0LIM -- v4.0 rebuilt 2026-06-17 (VehStolenRemoved mainline + PascalCase USx fields + restored RMS args); NOT yet imported, test pkg reset to T1 | VehicleStolenQuery NOT built (USER-APPROVED skip; state auto-runs QV, response data-mined via QRDM); VehReg 2 combos (RQ/RQN), poisoned-array RandomRequest=Y conditions removed (RandomRequest user-controlled in any[]); PascalCase USx fieldIds (CAD/OnScene), Mark43/RMS keys stay camelCase; RMS handler args restored (fixed _R $args collision, matches HIDLE baseline); CAD combo defaults; NCIC state; shared RMS module, RMS Vehicle stripped to 3 attrs |
| HI_HCJDC_OFML | providers/HI_HCJDC_OFML/ | v2.0 | 67P/0F/0W/0LIM -- v1.9 consolidated 2026-06-17 (single JSON; BASE/MC split retired); NOT yet imported, test pkg reset to T1 | 6 basic queries (Article/Boat/DH/DL/Gun/VehReg), 17 combos all reachable; single JSON; Person 3 cards (Search Options[State] + Driver License + Driver History, DH-suffix, one-directional deselect); Vehicle/Boat/Firearm/Article 1 card; Type Code dropdown (VEHICLE_TYPE/HI_NIBRS) per HIDLE ref; ImageIndicator=N combo defaults on all 6 VehReg combos (real CAD fix -- not card count); State in DL/DH any[] for OOS; labels: self-explanatory field labels + short card titles (v2.0 convention) |
