...
| FL_FCIC | providers/FL_FCIC/ | v7.4 | 92P/0F/0W/0LIM -- 30/30 combos, 6 QIDMs. v7.4 (2026-07-16) Vehicle card cosmetic pass + intentional combo removal (DEX-1279, Gordon Hallof UX feedback): card title -> "Vehicle Registration Search by License Plate, "OR" VIN, "OR" Decal"; dropped "(or search by VIN/Decal)"/"(out-of-state plates)"->"(out-of-state)"/"(optional)"->"(By VIN optional)" qualifiers on Plate/Make/Year; ImageIndicator -> "NCIC Image - if available" (FL-only, deliberately NOT a global rule change -- Gordon flagged portfolio-wide implications, discussion pending). REMOVED TitleLienInformation field + FRQTitleLienInformation combo entirely (was live/100%-covered) -- Gordon's explicit product decision to sunset that query path; documented as approved-skip in FL_FCIC_ACCEPTED_DIVERGENCES.txt + SQVR (31->30 combos). Vehicle re-opened for live re-test; Article/Boat preserved blocked at v7.1; Firearm/Person unaffected (open/PENDING from v7.3/v7.2). Prior -- v7.3 (Firearm card, DEX-1280) + v7.2 (Person DH card, DEX-1278): same card-title-carries-disambiguation pattern, verify_build CHECK 15 Rule 2 DH-label gate downgraded FAIL->Info portfolio-wide + Rule 3 Purpose Code exemption wired in. v7.1 (2026-06-30): Boat Hull>Reg guardrail completed for the FBQ in-state family, zero-error live pass certified (DEX-971). Full history: `providers/FL_FCIC/docs/tracking/CHANGELOG_FL_FCIC.md`. | 2-card: Vehicle(Options+Search), Person(DL+DH OOS-only), Boat(Options+Search). Devdoc combo order + EXISTENCE-ONLY routing conditions (State NOT_EXISTS / OLN NOT_EXISTS / RelatedHit NOT_EXISTS) for first-match + pool isolation; ALL value-comparison conditions removed (poisoned-array rule, QIDM_REFERENCE Sec 2a); DH KQ out-of-state only; DH+Boat destination state = NCIC dropdown; Boat QB stolen routing via relatedHitSearchIndicator in set[]; identifier-priority guardrails complete (Plate>VIN, OLN>Name, Hull>Reg); Attention auto-populated (handler); RMS Vehicle stripped to 3 attrs |
