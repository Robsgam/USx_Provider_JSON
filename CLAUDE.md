...
| NY_NYSPIN_EJUSTICE | providers/NY_NYSPIN_EJUSTICE/ | v4.3 | 81P/0F/0W/0LIM -- v4.3 (2026-07-07) restored MessageContinueKeyCode (a real DGRP metadata field wrongly dropped in v4.2 on a devdoc description -- metadata is the field-authority, not the devdoc) as a visible field; Age relabel "DOB Search Range (+/- years)" beside DOB retained. DGRP back to full 10-field metadata set. Built on v4.1 (DGRP broken out to its own DL NAME SEARCH card, DGRP-suffixed Name/DOB/Sex + address block, autoSelect=TRUE -- own field pool removes the v2.8 shared-Name co-fire cause, LIVE-VERIFY isolation; removed genuine over-inclusions Gun ImageIndicator + Boat RelatedHitSearchIndicator, which are NOT in metadata). Full re-test from T1 on re-import. Full history: `providers/NY_NYSPIN_EJUSTICE/docs/CHANGELOG_NY_NYSPIN_EJUSTICE.md`. | PascalCase, 9 cards (Veh 2, Per 4 [Options+DL+DL-Name-Search+DH], Gun 1, Art 1, Boat 2), 17 combos, 7 QIDMs, DGRP own card+full 10-field metadata set (autoSelect), DH OOS combos (DALHOUT/DALLOUT) with State EXISTS/NOT_EXISTS routing, Vehicle plate OOS combo (RVEHOUT), NyNyspinTransactionName visible on DH (default DALL), PurposeCode default C on DH OOS combos, Choice-set OOS pattern (LIMIT #36), DH-suffix+one-directional queriesToDeselect, CAD defaults, State no-default (LIMIT #30), identifier-priority guardrails (Plate>VIN, OLN>Name DL+DH, Hull>Reg) |
