...
(() => {
  ...
  window.__usxRunPlan = async function (plan, entityFilter, opts) {
    ...
    // `entityFilter` is really a GROUP filter and matches `tab` first (BUILD 2026-09-17e): one
    // targetEntity can host several tabs, so a plan emitted after 2026-09-17 names the owning
    // QUERYINPUTFORM per test ('Wanted Person', 'Administrative Message') and the panel offers
    // THOSE. Falling back to `t.entity` keeps older plans and the other 20 providers identical.
    const tests = plan.tests.filter((t) => (t.kind === 'combo' || t.kind === 'any' || t.kind === 'any-field' || t.kind === 'guardrail' || t.kind === 'value-strip') && (!entityFilter || (t.tab || t.entity) === entityFilter));
