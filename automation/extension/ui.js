...
(() => {
  ...
  function build(kind) {
    ...
    if (kind === 'admin') {
      ...
      depGo.onclick = async () => {
        // \u26a0\ufe0f RE-FETCH FIRST, ALWAYS. This used to act on the jobRow captured by depLoadJob() at
        // PAGE LOAD, so a job re-cut while the page sat open was invisible -- the operator pressed
        // the button and imported a SUPERSEDED build, or got a refusal whose reason was a stale
        // expectation rather than the tenant. That is the identical defect already fixed on the
        // read-side RUN THE JOB button; fixing it there and not here is how it survived.
        // 2026-09-16: it bit on SC_SLED. Three jobs were cut in one session (v1.1, v1.2, and a
        // re-cut after a probe import) and the panel still held the first.
        depStat.style.color = '#fa0'; depStat.textContent = 're-reading the job file\u2026';
        await depLoadJob();
    ...
    if (kind === 'dex') {
      ...
      fetchBatch.onclick = () => { if (!requireArmed()) return;
        ...
        // ⚠️ maxNew IS A WIRE-ROW BUDGET, NOT A TEST COUNT, AND SETTING IT TO `n` MADE EVERY
        // CO-FIRING PROVIDER TAKE THREE PRESSES. One submit can put SEVERAL rows on the wire --
        // an SC_SLED plate fires VehicleRegistration AND VehicleStolen -- so a 63-test run lays
        // down ~90 rows. Stopping after 63 fetched rows therefore runs out of budget BEFORE
        // reaching the rows belonging to the last tests, and the console says
        // "PARTIAL CAPTURE: 63 captured, 8 of 63 manifest entries NOT captured", which reads
        // exactly like a failure. Measured 2026-09-18: presses of 63 -> 8 -> 1 before it drained,
        // and Rob reported it twice as "fetch is failing again". It was not failing; it was
        // rationed. (The 09-17 run was the same cause and cost 14 stranded captures.)
        // x4 with a floor of 40: the largest co-fire in the portfolio is 3 queries on one submit
        // plus an RMS pair, and over-fetching is FREE here -- ids already captured are skipped by
        // the `have` set, so the only cost of a too-large budget is a few extra detail GETs, while
        // the cost of a too-small one is a sweep that looks broken.
        const o = { maxPages: 10, since: today }; if (n > 0) o.maxNew = Math.max(40, n * 4);
      ...
      function applyPlan(planObj, sourceName) {
        ...
        // GROUP BY TAB, NOT BY ENTITY (BUILD 2026-09-18h). A test's `entity` is its targetEntity,
        ...
        // emit_test_plan.ps1; `entity` is the fallback so plans predating it still populate.
        const groupOf = t => t.tab || t.entity;
        const entities = Array.isArray(tests) ? [...new Set(tests.map(groupOf).filter(Boolean))] : [];
      ...
      run.onclick = async () => { if (!requireArmed()) return;
        ...
        const tests = (plan.tests || []).filter(t => (t.kind === 'combo' || t.kind === 'any' || t.kind === 'any-field' || t.kind === 'guardrail') && (!entity || (t.tab || t.entity) === entity));
        if (!tests.length) { flash('No submittable tests found for "' + entity + '". Check the name (case-sensitive, e.g. Vehicle).'); return; }
      ...
      srcBtn.onclick = async () => {
        ...
        try {
          ...
          // ui.js has NO `L` alias -- that is driver.js's local. This file addresses the library
          // as window.__usxLib everywhere, and writing `L` here threw "L is not defined" at the
          // one moment the button mattered. Guarded rather than assumed present.
          if (!window.__usxLib || !window.__usxLib.triggerDownload) {
            throw new Error('usx_lib not loaded on this page -- cannot download');
          }
          window.__usxLib.triggerDownload(`usx_searchfe_${location.hostname}_${new Date().toISOString().replace(/[:.]/g,'-')}.json`,
