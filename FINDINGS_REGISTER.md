...
| **PICKLIST CAPTURE CURRENCY IS UNWATCHED — measured 2026-08-21** | `audit_picklist_scope` only asks whether `TENANT_PICKLISTS.json` EXISTS; nothing asks whether it still covers the form's current dropdowns. Across the 10 lifecycle-complete providers: **158 visible `FormSelect` controls / 21 with no captured option list**. Worst by far is **AZ_AZDPS — its capture covers ONE entity (Vehicle, 3 fields), so 13 of 16 dropdowns were never scoped**; the rest are controls added after the capture (NY 2, TX 2, FL 2, CA_CLETS 1, HI 1 — `RegistrationStateDH`, `ImageIndicatorDH`, `relatedHitSearchIndicator`). Every capture except NM's is stamped at an older version (TX v4.0 vs v4.21, HI v4.7 vs v4.20). **Impact is a RECORD gap, not a wire defect** — all 10 swept ALL-PASS, so the values used were accepted by the tenant; what is missing is the recorded proof of each dropdown's option set. IL/NJ/NM/OH are at 0, which is what proves the probe can pass | OPEN — needs a currency check + AZ re-capture at its next tenant visit |
| **`audit_query_trace` MISSING cannot tell "unbuilt" from "built under a synthetic keyRef"** | Of the **11 MISSING** across the 10 finished providers, **3 were pure artifacts**: FL `KQ set[OLN,State]` is built as `KQOperatorLicenseNumber`, NM `QUERY` (a `Choice` of OLN-vs-name) is built as `DL.OLN` + `DL.NAME`, NY `DALL[alt2/2]` is built as `DALLOUT`. Synthetic keyRef splits are a SANCTIONED pattern (LIMITATION #21/#36), so the gate flags the repo's own convention. This is why its output says "advisory -- adjudicate by hand" — but the artifact class is mechanical (match on the `set[]`, not the keyRef string) and could be removed | OPEN — tool improvement, no provider change |
