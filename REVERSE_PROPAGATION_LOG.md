...
| EMIT-GUARDRAIL-SIM | emit_test_plan guardrail expectedKeyRef now simulated (was a structural heuristic that mislabelled in-state/OOS split routing). Regenerate TEST_PLAN + re-run/re-validate guardrail routing. Test-harness fix, NO config change -- clear this flag after the guardrail re-validation passes. | HI_HCJDC_OFML | CA_CLETS, NJ_NJCJIS | NJ cleared 2026-07-02 (simulated expectedKeyRefs RANDFULL/FULLN/QBN match live v4.8 guardrail logs); pending: CA_CLETS (old heuristic mislabelled all 3 entity guardrails as OOS NLTS.* keyRefs; simulated plan expects in-state IA.QV/ID.L1/IR.QVC.O/IR.QVC.N/NLTS.KQ.O/IA.QB.H; NO live guardrail logs exist -- live guardrail run required) |
